...
if ($writer==""){
	if($_SESSION['manage_homest_idx']){ // 관리자 로그인 
	$writer = $_SESSION['manage_homest_name'];	
	} 
}
...
if($is_html != "Y"){
	...
	$max_query = "select max(ref) as max from board_content where 1=1 and bbs_code = '".$bbs_code."'";
	...
	$user_id = $_SESSION['manage_homest_id'];
	$view_id = $_SESSION['manage_homest_id'];
