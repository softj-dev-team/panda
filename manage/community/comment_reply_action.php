...
if ($passwd==""){
	$passwd = md5(sqlfilter($_SESSION['manage_homest_password']));	//비밀번호
} else {
...
$content = trim(sqlfilter($_REQUEST['content']));												//내용
	...
	$max_query = "select ref,step,depth from board_comment where 1=1 and idx = '".$idx."' ";
	...
	$reply_query = "update board_comment set step=(step+1) where board_tbname = '".$board_tbname."' and board_code = '".$board_code."' and ref=".$ref." and step>".$step;
	...
	<!--
	alert('댓글에대한 댓글 등록이 정상적으로 완료 되었습니다.');
