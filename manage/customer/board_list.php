...
$where = " and is_del='N' and step='0' and member_idx='".$_SESSION['manage_coinc_idx']."'";
...
<div id="wrap" class="skin_type01">
						...
						<tr>
							<?if($bbs_code == "faq"){?>
								...
								<td colspan="2">
									...
									?>
										<option value="<?=$row1['subject']?>" <?=$v_sect==$row1['subject']?"selected":""?>><?=$row1['subject']?></option>
									<?}?>
									</select>
								</td>
		...
		if($_include_board_is_notice == "Y"){ // 리스트 공지사항 보기가 가능한 게시판에 한하여 시작 
			...
			if(mysqli_num_rows($result_sub_notice)==0) {
				...
				for ($i_sub_notice=0; $i_sub_notice<mysqli_num_rows($result_sub_notice); $i_sub_notice++){ // 공지사항 지정글 루프
					...
					$sql_sub_notice_2 = "select board_title from board_config where 1 and board_code = '".$row_sub_notice['bbs_code']."' ";
					$query_sub_notice_2 = mysqli_query($gconnet,$sql_sub_notice_2);
					$row_sub_notice_2 = mysqli_fetch_array($query_sub_notice_2);

					$sql_sub_notice_file = "select file_org,file_chg from board_file where 1 and board_tbname='board_content' and board_code = '".$row_sub_notice['bbs_code']."' and board_idx='".$row_sub_notice['idx']."' order by idx asc limit 0,1";
					...
					$sql_sub_notice_3 = "select write_time from board_content where 1 and is_del='N' and p_no='".$row_sub_notice['idx']."'";
					$query_sub_notice_3 = mysqli_query($gconnet,$sql_sub_notice_3);
					$row_sub_notice_3 = mysqli_fetch_array($query_sub_notice_3);
		...
		?>
			<tr>
				...
				<?if($row_sub_notice_file['file_chg']){?>
					<a href="/pro_inc/download_file.php?nm=<?=$row_sub_notice_file['file_chg']?>&on=<?=$row_sub_notice_file['file_org']?>&dir=<?=$row_sub_notice['bbs_code']?>"><?//=$row_sub_notice_file['file_org']?><img src='/image/icon/ico_file.gif' border='0' align='absmiddle'></a>
				<?}?>
				</td>-->
				<td><?=$row_sub_notice[cnt]?></td>
				<td><?=substr($row_sub_notice[write_time],0,10)?></td>
				<?if($bbs_code == "qna"){?>
					<td><?=substr($row_sub_notice_3[write_time],0,10)?></td>
				<?}?>
			</tr>
	<?
				} // 공지사항 지정글 루프 종료 
	...
	?>	
			...
			for ($i=0; $i<mysqli_num_rows($result); $i++){
				...
				$sql_sub_3 = "select write_time from board_content where 1 and is_del='N' and p_no='".$row['idx']."'";
				//echo $sql_sub_3."<br>";
				$query_sub_3 = mysqli_query($gconnet,$sql_sub_3);
				$row_sub_3 = mysqli_fetch_array($query_sub_3);
			...
			?>
						...
						<?}?>
							...
							<?if($row_file['file_chg']){?>
								<a href="/pro_inc/download_file.php?nm=<?=$row_file['file_chg']?>&on=<?=$row_file['file_org']?>&dir=<?=$row['bbs_code']?>"><?//=$row_file['file_org']?><img src='/image/icon/ico_file.gif' border='0' align='absmiddle'></a>
							<?}?>
							</td>-->
							<td><?=$row[cnt]?></td>
							<td><?=substr($row[write_time],0,10)?></td>
							<?if($bbs_code == "qna"){?>
								<td><?=substr($row_sub_3[write_time],0,10)?></td>
							<?}?>
						</tr>
				<?}?>	
