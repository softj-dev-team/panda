...
function go_portpolio_3(){
	$("#s_gender").val($("#inr_s_gender").val());
	$("#s_level").val($("#inr_s_level").val());
	$("#field").val($("#inr_field").val());
	$("#keyword").val($("#inr_keyword").val());
	 var formData = $("#portfrm_3").serialize();
		$.ajax({
		type : "POST",
		url : "member_select_coupon.php",
		cache : false,
		data : formData,
		success : port_Success_3,
		error : port_Error_3
	});
}

function port_Success_3(json, status){
	$("#port_list_area3").html($.trim(json));
}

function port_Error_3(data, status){
	 alert("error");
}