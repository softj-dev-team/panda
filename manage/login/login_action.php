...
$sql = "select * from member_info where 1 and user_id='".$pm_id."' and memout_yn not in ('Y','S') and del_yn='N' and member_type in ('AD')";
//echo $sql;
...
if(mysqli_num_rows($result)>0){
	...
	if($_SERVER['REMOTE_ADDR'] == "59.9.33.3" || $_SERVER['REMOTE_ADDR'] == "220.116.84.234" ||$_SERVER['REMOTE_ADDR'] == "180.80.138.80"){
	} else {
	...
	$_SESSION['manage_coinc_id'] = $pm_id;
	$_SESSION['manage_coinc_idx'] = $row['idx'];
	$_SESSION['manage_coinc_name'] = $row['user_name'];
	$_SESSION['manage_coinc_password'] = $pm_pwd;
