...
//$sql = "SELECT * FROM send_msg where 1 and mail_method = '".$_SESSION['manage_homest_section']."' and mail_gubun = '".$mail_gubun."' and mail_key= '".$mail_key."' ";
$sql = "SELECT * FROM board_content where 1 and bbs_sect= '".$mail_key."' ";
...
<SCRIPT LANGUAGE="JavaScript">
	<!--
	alert('발송된 푸쉬가 없습니다.');
	location.href =  "push_send_list.php?<?=$total_param?>";
	//-->
</SCRIPT>
<?
exit;
...
<!--
	...
	function go_delete(no){
			...
			_fra_admin.location.href = "pushsend_delete_action.php?mail_key="+no+"&<?=$total_param?>";
			}
	}
		...
		<div class="container clearfix">
			...
			<table>
				...
				<?if($mail_gubun == "mail"){?>	
					...
					<?
							$board_content = stripslashes($row[content]);
							//$board_content = preg_replace("/ style=(\"|\')?([^\"\']+)(\"|\')?/","",$board_content);
							//$board_content = preg_replace("/ style=([^\"\']+) /"," ",$board_content); 
							$board_content = preg_replace("/ width=(\"|\')?\d+(\"|\')?/","",$board_content);
							$board_content = preg_replace("/ height=(\"|\')?\d+(\"|\')?/","",$board_content);
							$board_content = str_replace('"%"','"',$board_content);
							$board_content = str_replace("<img","<img style='max-width:100%;'",$board_content);
						?>
					<tr>
						<th >메일내용</th>
						<td width="*" colspan="3" style="padding-top:10px;padding-bottom:10px;"><?=$board_content?></td>
					</tr>
				<?}elseif($mail_gubun == "push"){?>	
					<tr>
						<th >제목</th>
				...
				<?}?>
					<tr>
						<th >발송일시</th>
						<td width="*" colspan="3"><?=$row[write_time]?></td>
					</tr>
			...
			<h3 style="margin-top:10px;">총 <?=$num_sub?> 명의 회원에게 이 <?=$mail_gubun_str?>를 발송했습니다.</h3>
