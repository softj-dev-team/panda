...
<?
	...
	$s_buymtd = trim(sqlfilter($_REQUEST['s_buymtd'])); 
	$s_buysts = trim(sqlfilter($_REQUEST['s_buysts']));
	...
	$target_param = "member_idx=".$member_idx."&v_sect=".$v_sect."&date_s=".$date_s."&date_e=".$date_e."&s_sect1=".$s_sect1."&s_sect2=".$s_sect2."&s_protype=".urlencode($s_protype_arr)."&s_buymtd=".$s_buymtd."&s_buysts=".$s_buysts."&keyword=".$keyword."&s_cnt=".$s_cnt."&s_order=".$s_order;

	if(!$v_sect){
		$v_sect = "general";
	}

	$where = " and is_del='N' and member_idx_buy='".$member_idx."'";
...
?>
	...
	<ul class="list_tab" style="margin-top:10px;margin-bottom:10px;padding-left:10px;">
		<li id="member_view_buy_1" class="list_tab_tab_tab<?if($v_sect == "general"){?> on<?}?>">
			<a href="javascript:member_tab_buy('general');">일반 구매이력</a>
		</li>
		<li id="member_view_buy_2" class="list_tab_tab_tab<?if($v_sect == "auction"){?> on<?}?>">
			<a href="javascript:member_tab_buy('auction');">경매 구매이력</a>
		</li>
	</ul>
	
	<form name="inner_s_buy" id="inner_s_buy">
	...
	<div class="list">
		...
		<table class="search">
			...
			<?if($v_sect == "general"){?>
				<tr>
					...
					<td colspan="5">
					<? foreach ($arr_buy_status_gen as $key=>$val) {
							if($s_buysts == $key){
								$check = "checked";
							} else {
								$check = "";
						...
						?>
						<input type="radio" id="s_buysts_<?=$key?>" name="s_buysts" <?=$check?> value="<?=$key?>"/> <?=$val?> &nbsp; 
					<?}?>
					</td>
				</tr>
			<?}elseif($v_sect == "auction"){?>
				<tr>
					...
					<td colspan="5">
					<? foreach ($arr_buy_status_auc as $key=>$val) {
							if($s_buysts == $key){
								$check = "checked";
							} else {
								$check = "";
		...
		<div class="align_r mt20">
			<button class="btn_search" type="button" onclick="sch_mem_view_buy();">검색</button>
		</div>
		...
		<div class="search_wrap">
			<!-- 목록 옵션 시작 -->
			<div class="result">
				<p class="txt">일반구매 총 <span><?=number_format($num)?></span>건</p>
				<div class="btn_wrap">
					<select id="s_cnt" name="s_cnt" onchange="sch_mem_view_buy();">
			...
			<table class="search_list">
				...
				<thead>
					<tr>
						...
						<th scope="col">작품명</th>
						<th scope="col">판매자</th>
						<th scope="col"><?if($v_sect == "general"){?>구매단계<?}elseif($v_sect == "auction"){?>경매단계<?}?></th>
						<th scope="col">결제일</th>
				...
				<? if($num==0) { ?>
					<tr>
						<td colspan="10" height="40">구매이력이 없습니다.</strong></td>
					</tr>
				<? } ?>
				<?
				...
				?>
					<tr>
						<td><?=$listnum?></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=$row['product_title']?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=$row['user_sale']?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');">
						<?if($v_sect == "general"){?>
							<?=$arr_sale_status2_gen[$row['sale_status']]?>
						<?}elseif($v_sect == "auction"){?>
							<?=$arr_sale_status2_auc[$row['sale_status']]?>
						<?}?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=substr($row['payment_date'],0,10)?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=get_payment_method($row['pay_sect_1'])?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');">
						...
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=get_order_status($row['orderstat'])?></a></td>
						<td><a href="javascript:go_buy_view('<?=$row['idx']?>');"><?=substr($row['cancel_date'],0,10)?></a></td>
					</tr>
				<?}?>
			</table>
			...
			<?
				$target_link = "inner_member_view_buy.php";
...
<script>
	...
	function go_buy_view(idx){
		//location.href = 
		<?if($v_sect == "general"){?>
			window.open("popup_buy_offer_view.php?idx="+idx+"","buyview", "top=100,left=100,scrollbars=yes,resizable=no,width=1080,height=600");
		<?}elseif($v_sect == "auction"){?>
			window.open("popup_buy_auction_view.php?idx="+idx+"","buyview", "top=100,left=100,scrollbars=yes,resizable=no,width=1080,height=600");
		<?}?>
	}
</script>