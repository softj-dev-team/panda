...
<?
	...
	$com_status = trim(sqlfilter($_REQUEST['com_status']));
	...
	if($prev_cnt == 0){
		error_frame("설정을 변경할 가맹점이 존재하지 않습니다.");
		exit;
	}
	...
	$query2 = " update member_info_company set ";
	$query2 .= " com_status = '".$com_status."', ";
	$query2 .= " status_date_".$com_status." = now() ";
	$query2 .= " where member_idx = '".$idx."' ";
