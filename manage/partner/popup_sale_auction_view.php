...
<?
	...
	// 오퍼이력 쿼리 시작 
		...
		$query = "select *,(select user_nick from member_info where 1 and del_yn='N' and idx=product_info_sale_auction.member_idx) as user_nick from product_info_sale_auction where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
		$result = mysqli_query($gconnet,$query);

		$query_cnt = "select idx from product_info_sale_auction where 1 ".$where;
	...
	$query_last = "select *,(select user_nick from member_info where 1 and del_yn='N' and idx=product_info_sale_auction.member_idx) as user_nick from product_info_sale_auction where 1 and auction_yn='Y' and is_del='N' and sale_idx='".$row_basic['idx']."'";
			  ...
			  <!-- 기본정보 시작 -->
				<div class="write">
					<table>
							...
							<tr>
								<td colspan="4" style="background-color:#ffffff;">[<?=$row_basic['product_title']?>] 판매내역 보기</td>
							</tr>
							<tr>
								<th scope="row">판매자</th>
								<td style="background-color:#ffffff;"><?=$row_basic['user_sale']?></td>
								<th scope="row">경매기간</th>
								<td style="background-color:#ffffff;"><?=$row_basic['auc_sdate']?> ~ <?=$row_basic['auc_edate']?></td>
							</tr>
					</table>
				</div>	
				<!-- 기본정보 종료 -->
				...
				<div class="list" style="margin-top:-50px;">
					<div class="search_wrap">
						<table class="search_list">
							...
							<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">경매입찰 내역이 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
			...
			<?if(mysqli_num_rows($result_last) > 0){
				...
				<p class="tit">낙찰정보</p>
					<table>
							...
							<tr>
								<th scope="row">낙찰자</th>
								<td style="background-color:#ffffff;"><?=$row_last['user_nick']?></td>
								<th scope="row">낙찰금액</th>
							...
							<tr>
								<th scope="row">입찰일시</th>
								<td style="background-color:#ffffff;"><?=$row_last['wdate']?></td>
								<th scope="row">낙찰일시</th>
								<td style="background-color:#ffffff;"><?=$row_last['aucdate']?></td>
							</tr>
						</table>
					</div>	
				<!-- 낙찰내역 종료 -->
