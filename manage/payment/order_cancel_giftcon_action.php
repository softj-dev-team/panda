...
if($session_admin_idx && strpos($_SERVER["HTTP_REFERER"], "/html/mainnet/manage/payment/") !== false ){
	$is_admin = true;
}else{
	$is_admin = false;
}
...
$sql_pre = "select * from ".NS."order_member where order_num='".$order_num."' and orderstat in ('com','can1','pre') and order_gift_yn = 'Y'";
...
$xml_url2  = "http://mgpay.co.kr/mecon_api/cancel.php";
	
$query_js_pro = "select idx,pro_code from order_product where order_num = '".$order_num."'";
$result_js_pro = mysqli_query($GLOBALS['gconnet'],$query_js_pro);
...
for($result_js_i=0; $result_js_i<mysqli_num_rows($result_js_pro); $result_js_i++){ // 상품숫자만큼 루프 시작
	...
	$xml_data_2 = array(
		"tr_id"=>$order_num, 
		"goods_id"=>$result_js_row[pro_code] 
	);
		
	$response_2 = get_curl_xml_post($xml_url2, $xml_data_2);
	$rxml_2 = simplexml_load_string($response_2) or die ("Error: Cannot create object");
	$mms_result = $rxml_2->result->StatusCode;
	$mms_result_txt = $rxml_2->result->StatusText;

	if($mms_result == "0"){ // 결제취소 성공이면 시작 
		$app_pay = 1;
		
		$query_in_mpro = " update ".NS."log_buy_mobile_coupon set "; 
		$query_in_mpro .= " status = '9' ";
		$query_in_mpro .= " where tr_id = '".$order_num."' and goods_id='".$result_js_row[pro_code]."' ";
		$result_in_mpro = mysqli_query($GLOBALS['gconnet'],$query_in_mpro);
...
if($app_pay == 1){ // 결제 취소 API 가 성공일때 시작
	...
	<?}?>
<?
} else { // 결제 취소 API 가 성공일때 종료, 실패일때 시작
			$query = " update ".NS."order_member set "; 
			$query .= " cancel_memo = '".$mms_result_txt."' ";
			$query .= " where order_num = '".$order_num."' ";
			if(!$result = mysqli_query($GLOBALS['gconnet'],$query)){
				error_frame("취소가 실패했습니다.");
			}
			...
			$query_pro = " update ".NS."order_product set "; 
			$query_pro .= " cancel_memo = '".$mms_result_txt."' ";
			$query_pro .= " where order_num = '".$order_num."' ";
			if(!$result_pro = mysqli_query($GLOBALS['gconnet'],$query_pro)){
				error_frame("취소가 실패했습니다.");
			}
			...
			$cancel_error_msg = iconv("euc-kr","utf-8",$mms_result_txt);

			if($is_admin){
		?>
		<script type="text/javascript">
			alert("<?=$cancel_error_msg?> 로 인해 결제취소가 실패 하였습니다.");
			parent.location.href =  "order_list.php?<?=$total_param?>";
		</script>
		<?}else{?>
		<script type="text/javascript">
