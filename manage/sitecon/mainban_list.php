...
<? include $_SERVER["DOCUMENT_ROOT"]."/pro_inc/include_htmlheader_manage.php"; // 배너페이지 헤더?>
<? include $_SERVER["DOCUMENT_ROOT"]."/manage/include/check_login.php"; // 배너 로그인여부 확인?>
<?
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list">
					...
					<!--<select name="v_sect" size="1" style="vertical-align:middle;width:50%;" >
						...
						<? } elseif($s_sect1 == "mobile"){ ?>
							<option value="메인화면 중앙롤링" <?=$v_sect=="메인화면 중앙롤링"?"selected":""?>>메인화면 중앙롤링</option>
							<option value="좌측메뉴 상단 롤링" <?=$v_sect=="좌측메뉴 상단 롤링"?"selected":""?>>좌측메뉴 상단 롤링</option>
							<option value="메인하단 롤링" <?=$v_sect=="메인하단 롤링"?"selected":""?>>메인하단 롤링</option>
						<? } ?>
					</select>-->
			...
			<table class="search_list">
				...
				<input type="hidden" name="total_param" value="<?=$total_param?>"/>
					...
					<tr>
						<td><?=$listnum?></td>
						<!--<td ><a href="javascript:go_view('<?=$row[idx]?>');"><?//=$row[main_sect]?><?=$section?></a></td>-->
						<td style="padding-top:10px;padding-bottom:10px;">
					<?if($row[section] == "img"){?>
						<?if($row[file_c] != "" && $row[file_c] != " "){?>
							<a href="javascript:go_view('<?=$row[idx]?>');"><img src="<?=$_P_DIR_WEB_FILE?>main_banner/img_thumb/<?=$row[file_c]?>" border="0" style="max-width:90%;"></a>
						<?}?>
					...
					<?}?>
						...
						<td ><a href="javascript:go_view('<?=$row[idx]?>');"><?=$row['main_title']?></a></td>
