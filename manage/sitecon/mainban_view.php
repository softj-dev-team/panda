...
//$sql = "SELECT * FROM mainban_info where 1=1 and idx = '".$idx."' and section = '".$_SESSION['manage_homest_section']."' ";
					...
					<tr>
						...
						<td colspan="3" style="padding-left:10px;padding-top:10px;padding-bottom:10px;">
							<?if($row[file_c] != "" && $row[file_c] != " "){?>
								<a href="javascript:view_pic('../product/pro_picview.php?idx=<?=$row[idx]?>&tbn=mainban_info&pic_path=main_banner&mode=c');"><img src="<?=$_P_DIR_WEB_FILE?>main_banner/img_thumb/<?=$row[file_c]?>" style="border:0;max-width:90%;"></a>
							<?}?>
						</td>
					</tr>
