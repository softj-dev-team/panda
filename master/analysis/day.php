...
$startDate = isset($_REQUEST['s_date']) ? new DateTime($_REQUEST['s_date'])  : (clone $currentDate)->modify('-1 day');
...
$query = "
        SELECT
        DATE(log_table.fsenddate) AS send_date,
    ...
    $where
    GROUP BY
        DATE(log_table.fsenddate)  -- 일별로 그룹화
           , sms_save.module_type
    ORDER BY
        send_date desc,sms_save.module_type desc
    ";
