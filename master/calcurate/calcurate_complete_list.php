...
$order_by = " order by vdate asc ";

if($s_group){
	$query = "select *,(select user_name from member_info where 1 and member_type = 'PAT' and idx=sil_change_money.partner_idx) as user_name,(select user_id from member_info where 1 and member_type = 'PAT' and idx=sil_change_money.partner_idx) as user_id from sil_change_money where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;

	//echo $query;

	$result = mysqli_query($gconnet,$query);

	$query_cnt = "select idx from sil_change_money where 1 ".$where;
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<!-- 검색창 시작 -->
				<table class="search">
				<form name="s_mem" id="s_mem" method="post" action="calcurate_complete_list.php">
						...
						<tr>
							<th scope="row">히얼업 검색</th>
							<td colspan="2">
								<select name="s_group" style="vertical-align:middle;" onchange="s_mem.submit();">
									<option value="">선택하세요</option>
									<?php
										$sub_sql = "select idx,user_name from member_info where 1 and memout_yn != 'Y' and memout_yn != 'S' and member_type='PAT' and del_yn='N' order by user_name asc";
			...
			<!-- 엑셀 출력을 위한 전송 폼 시작 -->
			<form name="order_excel_frm" id="order_excel_frm" method="post" action="calcurate_complete_excel_list.php">
			<input type="hidden" name="mode" value="ser">
			<input type="hidden" name="v_sect" value="<?php echo $v_sect?>"/>
		...
		<!--<form name="frm_calc_com" id="frm_calc_com" method="post" action="calcurate_all_ok.php"  target="_fra_admin">
			...
			<table class="search_list">
				...
				<thead>
					<tr>
						<th width="10%">신청일시</th>
						<th width="9%">히얼업 이름</th>
						<th width="9%">히얼업 아이디</th>
						<th width="9%">아티스트 회신율</th>
						<th width="9%">신청실링</th>
						<th width="9%">환급은행</th>
						<th width="9%">환급계좌</th>
						<th width="9%">예금주</th>
						<th width="9%">송금금액</th>
						<th width="9%">송금일자</th>
						<th width="9%">송금처리 관리자 ID</th>
					</tr>
				</thead>
				<tbody>
			<?php if(!$s_group){?>
				<tr>
					<td colspan="10" height="40"><strong>히얼업 선택하여 검색해 주세요.</strong></td>
				</tr>
		   <?php } else {?>
				<?php  if($num==0) { ?>
					<tr>
						<td colspan="10" height="40"><strong>환급완료 송금 내역이 없습니다.</strong></td>
					</tr>
				<?php  } ?>
				...
				?>	
					<tr>
						...
						<td><?php echo number_format($row[send_money])?> 원</td>
			...
			?>	
				...
				<tr>
					...
					<td></td>
					<td></td>
					<td><?php echo number_format($total_send_m)?> 원</td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
