...
<? include $_SERVER["DOCUMENT_ROOT"]."/weddingcrea/master/include/check_login_frame.php"; // 관리자 로그인여부 확인?>
...
$board_align = trim(sqlfilter($_REQUEST['board_align']));
		...
		$sql_pre1 = "select idx from board_config where 1=1 and board_code = '".$board_code."' "; // 중복 코드 방지
		...
		if($board_master_idx){ // 게시판 관리자를 입력했을때 시작
	...
	if ($_FILES['file2']['size']>0){
		...
		//$watermark_sect = "imgw";
		$watermark_sect = "";
		$file_c = uploadFileThumb_1($_FILES, "file2", $_FILES['file2'], $_P_DIR_FILE,$i_width,$i_height,$i_width2,$i_height2,$watermark_sect);*/
		$file_c2 = uploadFile($_FILES, "file2", $_FILES['file2'], $_P_DIR_FILE); // 파일 업로드후 변형된 파일이름 리턴.
	}

		$is_del = "N";

		$query = " insert into board_config set "; 
