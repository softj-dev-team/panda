...
if($_SESSION['member_homest_idx'] == $row['member_idx']){ 
	...
	$sql_prev = "select idx from board_view_cnt where 1=1 and board_tbname='board_content' and board_code = '".$row['bbs_code']."' and board_idx='".$row['idx']."' and member_idx = '".$_SESSION['member_homest_idx']."' ";
	...
	if($cnt_prev == 0){ // 현 게시물을 처음 볼때 한해서 조회수를 증가시킨다 시작 
			...
			$query_view_cnt .= " member_idx = '".$_SESSION['member_homest_idx']."', ";
...
<section id="content">
	<div class="inner">
		<h3><?=$bbs_str?> 게시물 내용보기 <?//=$sql_prev?></h3>
		<div class="cont">
			<table class="t_view">
				<colgroup>
					<col width="10%" />
			...
			<form name="pay_back_frm" action="board_view_action.php" target="_fra_admin" method="post" >
				...
				<th >원고료 책정</th>
				<td colspan="3">
					본 게시물에 대해서 원고료로 <input type="text" style="width:10%;" name="pay_bak" required="yes"  message="원고료" is_num="yes" value="<?=$row['pay_bak']?>"> 박을 책정합니다.
					&nbsp;  <a href="javascript:pay_back_submit();" class="btn_blue2">고료책정</a>
				</td>
				</tr>
			...
			<table width="100%" align="center">
				<tr>
					<td align="left" style="padding-left:20px;">
						<!-- 목록 -->
						<a href="javascript:go_list();" class="btn_blue2">목록보기</a>
						<a href="javascript:go_reco();" class="btn_blue2">추천하기</a>
					...
					<? if($_AUTH_REPLY){ // 덧글작성 권한 있을때 ?>	
						<!-- 답글 -->
						<a href="javascript:go_reply('<?=$row[idx]?>');" class="btn_blue2">덧글달기</a>
					<?}?>
					<? if($_AUTH_WRITE){ // 본문작성 권한 있을때 ?>	
						<!-- 삭제 -->
						<?if($row[is_del] != "Y"){?>
						<a href="javascript:go_delete('<?=$row[idx]?>');" class="btn_blue2">숨기기</a>
						<?}?>
						<?if($row[is_del] == "Y"){?>
						<!-- 삭제취소 -->
						<a href="javascript:go_delete_can('<?=$row[idx]?>');" class="btn_blue2">숨김해제</a>	
						<?}?>
					<?}?>
					</td>
					...
					<? if($_AUTH_WRITE){ // 본문작성 권한 있을때 ?>	
						<!-- 완전삭제 -->
						<a href="javascript:go_delete_complete('<?=$row[idx]?>');" class="btn_blue2">완전삭제</a>	
					<?}?>
					</td>
				</tr>
			</table>
	...
	<?if($_include_board_is_comment == "Y"){ // 한줄댓글 가능한 게시판일때 시작 ?>		
		...
		<?
			...
			$order_by_sub = " ORDER BY ref asc, step asc, depth asc ";
			...
			<tr>
				...
				<? } ?>		
					&nbsp;&nbsp; 
						...
						<?}?>
						&nbsp;<a href="javascript:comment_delete(<?=$row_sub[idx]?>);" class="btn_blue2">완전삭제</a>	
					 <?if($row_sub[file_chg]){?>
			...
			<form name="comment_reply_frm_<?=$row_sub[idx]?>" action="comment_reply_action.php" target="_fra_admin" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="passwd" value="<?=$_SESSION['admin_homest_password']?>">

					<tr id="comment_in_<?=$row_sub[idx]?>_1" style="display:none;padding-left:30px;">
						<th > 작성자</th>
						<td ><input type="text" name="writer" id="writer" style="width:200px;" value="<?=$_SESSION['admin_homest_name']?>" required="yes" message="한줄댓글 작성자"></td>
					</tr>
					...
					<tr id="comment_in_<?=$row_sub[idx]?>_3" style="display:none;padding-left:30px;">
						<th > 이미지 첨부</th>
						<td ><input id="file1" name="file1" type="file" style="width:300px;" /> &nbsp;&nbsp; <a href="javascript:go_comment_reply('comment_reply_frm_<?=$row_sub[idx]?>');" class="btn_blue2">댓글 등록</a> </td>
					</tr>
	...
	<? if($_AUTH_REPLY){ // 댓글작성 권한 있을때 ?>
		...
		<input type="hidden" name="passwd" value="<?=$_SESSION['admin_homest_password']?>">
			...
			<tr>
				<th >이미지 첨부</th>
				<td ><input id="file1" name="file1" type="file" style="width:300px;" /> &nbsp;&nbsp; <a href="javascript:go_submit();" class="btn_blue2">댓글 등록</a></td>
			</tr>
