...
$sql = "SELECT *,(select user_id from member_info where 1 and idx=compet_info.member_idx) as com_id,(select file_chg from board_file where 1 and board_tbname='compet_info' and board_code='file' and board_idx=compet_info.idx order by idx asc limit 0,1) as file_chg FROM compet_info where 1 and idx = '".$idx."' and pstatus='com' and is_del = 'N'";
...
<script type="text/javascript">
	function go_view(no){
		location.href = "compet_view.php?idx="+no+"&<?=$total_param?>";
	}
	
	function go_modify(no){
		location.href = "compet_modify.php?idx="+no+"&<?=$total_param?>";
	}

	function go_delete(no){
		if(confirm('공모전 정보를 삭제하시면 다시는 복구가 불가능 합니다. 정말 삭제 하시겠습니까?')){
			if(confirm('삭제하신 데이터는 복구할수 없도록 영구 삭제 됩니다. 그래도 삭제 하시겠습니까?')){	
				_fra_admin.location.href = "compet_delete_action.php?idx="+no+"&<?=$total_param?>";
			}
		}
	}
...
<div id="wrap" class="skin_type01">
				...
				<div class="write">
					...
					<table>
						...
						<tr>
							...
							<td colspan="3"><?=$row['com_name']?></td>
						</tr>
						<tr>
							<th scope="row">회사 및 단체 소개</th>
							<td colspan="3"><?=nl2br($row['com_info'])?></td>
						</tr>
						<tr>
							<th scope="row">업종</th>
							<td colspan="3"><?=get_selected_opt_name($row['idx'],"section")?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3"><?=$row[compet_sdate]?> ~ <?=$row[compet_edate]?></td>
						</tr>
						<tr>
							<th scope="row">공모전 상세정보</th>
							<td colspan="3"><?=nl2br($row['compet_detail'])?></td>
						</tr>
						<?
							$sect_sql = "select file_chg,file_org from board_file where 1 and board_tbname='compet_info' and board_code='docu' and board_idx='".$row['idx']."' order by idx asc";
						...
						?>	
							<tr>
								...
								<td colspan="3">
									<a href="/pro_inc/download_file.php?nm=<?=$sect_row['file_chg']?>&on=<?=$sect_row['file_org']?>&dir=compet_info"><?=$sect_row['file_org']?></a>
								</td>
							</tr>
						<?}?>
						<tr>
							<th scope="row">기본옵션</th>
							<td colspan="3"><?=get_selected_opt_name($row['idx'],"basic")?></td>
						</tr>
						<tr>
							<th scope="row">패키지 옵션</th>
							<td colspan="3"><?=get_compet_package_name($row['com_package'])?></td>
						</tr>
						<tr>
							<th scope="row">1위 상금</th>
							<td colspan="3">
								<?=number_format($row['compet_first_price'])?> 원
							</td>
						</tr>
					<?if($row['compet_second_ok'] == "Y"){?>
					<tr>
						<th scope="row">2위 상금</th>
						<td>
							<?=number_format($row['compet_second_price'])?> 원
						</td>
						<th scope="row">3위 상금</th>
						<td>
							<?=number_format($row['compet_third_price'])?> 원
						</td>
					</tr>
					<?}?>
					<tr>
						<th scope="row">공모전 진행기간</th>
						<td colspan="3"><?=get_selected_opt_name($row['idx'],"period")?></td>
					</tr>
					<tr>
						<th scope="row">노출옵션</th>
						<td colspan="3"><?=get_selected_opt_name($row['idx'],"display")?></td>
					</tr>	
					<tr>
						<th scope="row">조회수</th>
						<td><?=number_format($row[vcnt])?> 회</td>
						<th scope="row">참여자</th>
						<td><?=number_format($row[rcnt])?> 명</td>
					</tr>
					<tr>
						<th scope="row">진행상태</th>
						<td colspan="3"><?=get_compet_rstatus($row['rstatus'])?></td>
					</tr>	
					<tr>
	...
	<form name="s_mem" method="post" action="../compet_regist/regist_list.php" target="_self">
		<input type="hidden" name="bmenu" value="5"/>
		<input type="hidden" name="smenu" value="1"/>
		<input type="hidden" name="field" value="compet_title"/>
		<input type="hidden" name="keyword" value="<?=$row['compet_title']?>"/>
	</form>

					<div class="write_btn align_l">
						<a href="javascript:s_mem.submit();" class="btn_gray">참가작 리스트</a>
						<a href="../compet_regist/regist_write.php?bmenu=5&smenu=1&compet_idx=<?=$row['idx']?>" class="btn_green">참가작 출품</a>
					</div>
				...
				<!--
					...
					<colgroup>
						...
						<form name="frm" action="compet_view_action.php" target="_fra_admin" method="post" >
				...
				<form name="frm_chk_regist2" id="frm_chk_regist2" action="compet_ticket_regist_action.php" target="_fra_admin" method="post" >
					<input type="hidden" name="compet_info_idx" value="<?=$row['idx']?>"/>
			...
			<?
			$where_sub = " and compet_info_idx='".$row['idx']."' and is_del = 'N' and view_ok = 'Y'";
			...
			$sub_sql = "select * from compet_info_ticket where 1 ".$where_sub.$order_by_sub." limit ".$StarRowNum_sub." , ".$EndRowNum_sub;
			$sub_query = mysqli_query($gconnet,$sub_sql);
			$sub_cnt = mysqli_num_rows($sub_query);

			$query_sub_cnt = "select idx from compet_info_ticket where 1 ".$where_sub;
			...
			?> 
				<form name="frm_cate1_<?=$sub_i?>" method="post" action="compet_ticket_modify_action.php"  target="_fra_admin" enctype="multipart/form-data">
			...
			// 정상적으로 검색이 완료됐으면 
			 if (status === daum.maps.services.Status.OK) {
					...
					var infowindow = new daum.maps.InfoWindow({
						content: '<div style="width:150px;text-align:center;padding:6px 0;"><?=$row[compet_title]?></div>'
					});
