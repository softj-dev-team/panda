...
if($s_gubun){
	$where .= " and wdate >= '".$s_gubun."' ";
}

if($s_level){
	$where .= " and wdate <= '".$s_level."' ";
}

if($s_sect1 == "select_i"){
	$where .= " and select_first='N' and select_champ='N'";
} elseif($s_sect1 == "select_f"){
	$where .= " and select_first='Y' and select_champ='N'";
} elseif($s_sect1 == "select_c"){
	$where .= " and select_first='Y' and select_champ='Y'";
}
...
if ($field && $keyword){
	if($field == "compet_title"){
		$where .= "and compet_idx in (select idx from compet_info where 1 and ".$field." like '%".$keyword."%' and pstatus='com' and is_del = 'N')";
	} else {
		$where .= "and ".$field." like '%".$keyword."%'";
	}
}

$query_cnt = "select idx from compet_regist_info where 1 ".$where;
...
if($s_order == 1){ // 책장 올림차순 
	...
	$order_by = " order by regist_title asc "; 
} elseif($s_order == 3){ // 프로그램명 내림차순 
	$order_by = " order by regist_title desc "; 
} elseif($s_order == 4){ // 최신 등록순 
	$order_by = " order by align desc "; 
}

$query = "select *,(select user_id from member_info where 1 and idx=compet_regist_info.member_idx) as com_id,(select file_chg from board_file where 1 and board_tbname='compet_regist_info' and board_code='list' and board_idx=compet_regist_info.idx order by idx asc limit 0,1) as file_chg,(select compet_title from compet_info where 1 and idx=compet_regist_info.compet_idx) as compet_title from compet_regist_info where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
...
<!--	 
	function go_view(no){
		location.href = "regist_view.php?idx="+no+"&pageNo=<?=$pageNo?>&<?=$total_param?>";
	}
	...
	function go_regist(){
		location.href = "regist_write.php?<?=$total_param?>";
	}

	function go_regist_add(no){
		location.href = "regist_write_add.php?regist_idx="+no+"&<?=$total_param?>";
	}
	...
	if(check) {
		if(confirm('선택하신 프로그램를 삭제 하시겠습니까?')){
			if(confirm('삭제하신 프로그램는 복구할 수 없습니다. 정말 삭제 하시겠습니까?')){
				frm.action = "regist_list_action_del.php";
				frm.submit();
			}
		}
	} else {
...
var check = chkFrm('frm');
	if(check) {
		...
		frm.action = "regist_list_action_change.php";
		frm.submit();
	} else {
		false;
	}
...
//var check = chkFrm('frm');
	//if(check) {
		frm.action = "regist_list_action_excel.php";
		frm.submit();
	/*} else {
		false;
	}*/
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<!-- 검색창 시작 -->
				<table class="search">
				<form name="s_mem" method="post" action="regist_list.php">
						...
						<tr>
							...
							<td colspan="2">
								<select name="s_sect1" size="1" style="vertical-align:middle;" >
									...
									<option value="select_i" <?=$s_sect1=="select_i"?"selected":""?>>선정대기</option>
									<option value="select_f" <?=$s_sect1=="select_f"?"selected":""?>>1차선정</option>
									<option value="select_c" <?=$s_sect1=="select_c"?"selected":""?>>우승작</option>
								</select>
							</td>
							...
							<td colspan="2">
								<select name="field" size="1" style="vertical-align:middle;width:35%;">
									...
									<option value="member_name" <?=$field=="member_name"?"selected":""?>>디자이너 닉네임</option>
									<option value="work_detail" <?=$field=="work_detail"?"selected":""?>>작품설명</option>
								</select>
					...
					<table class="search_list">
							...
							<thead>
								<tr>
									...
									<th scope="col">미리보기 이미지</th>
									<th scope="col">디자이너 닉네임</th>
						...
						<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">등록된 참가작이 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
						...
						<tr>
							...
							<?if($row['file_chg']){?>
								<img src="/upload_file/compet_regist_info/img_thumb/<?=$row['file_chg']?>" style="max-width:90%;">
							<?}?>
							</a></td>
