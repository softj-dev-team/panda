...
$sql = "SELECT *,(select file_chg from board_file where 1 and board_tbname='curri_info' and board_code='sphoto' and board_idx=curri_info.idx order by idx asc limit 0,1) as file_chg FROM curri_info where 1 and idx = '".$idx."' and is_del = 'N'";
...
<script type="text/javascript">
	function go_view(no){
		location.href = "curri_view.php?idx="+no+"&<?=$total_param?>";
	}
	
	function go_modify(no){
		location.href = "curri_modify.php?idx="+no+"&<?=$total_param?>";
	}

	function go_delete(no){
		if(confirm('대주제 정보를 삭제하시면 다시는 복구가 불가능 합니다. 정말 삭제 하시겠습니까?')){
			if(confirm('삭제하신 데이터는 복구할수 없도록 영구 삭제 됩니다. 그래도 삭제 하시겠습니까?')){	
				_fra_admin.location.href = "curri_delete_action.php?idx="+no+"&<?=$total_param?>";
			}
		}
	}
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list_tit">
					<h3>등록된 대주제정보 상세보기</h3>
				</div>
				<div class="write">
					<p class="tit" style="background-image:url(../images/common/play.png); background-repeat:no-repeat; background-position:left center; font-size:16px; color:#454545; padding-left:22px; margin-bottom:10px;">대주제 등록정보</p>
					<table>
					...
					<tr>
						<th scope="row">설명글</th>
						<td colspan="3">
							<?=nl2br(stripslashes($row['curri_detail']))?>
						</td>
					</tr>
					...
					<!--<tr>
						...
						<td colspan="3">
							<?if($row[file_chg_over]){?>
								<img src="<?=$_P_DIR_WEB_FILE?>curri_info/<?=$row['file_chg_over']?>" style="max-width:90%;">
							<?}?>
						</td>
					</tr>-->
					<tr>
						<th scope="row">조회수</th>
						<td>
							<?=number_format(get_vcnt("curri_info","",$row['idx']))?> 회</td>
						</td>
					...
					<!-- 소주제 리스트 시작 -->
					<div class="search_wrap" id="exam_list_movie_area">
						<!-- exam_list_movie.php 에서 불러옴 -->
					</div>
					<!-- 소주제 리스트 종료 -->	
...
function go_check_submit2(){
	...
	function exam_list_movie_list(){
		get_data("exam_list_movie.php","exam_list_movie_area","curri_info_idx=<?=$row['idx']?>&v_sect=<?=$v_sect?>");
	}

	function exam_list_view(lectidx){
		window.open("exam_view.php?lecture_idx="+lectidx+"","examview", "top=100,left=100,scrollbars=yes,resizable=no,width=1080,height=600");
	}

	$(document).ready(function() {
		exam_list_movie_list();
	});
//-->
</script>
			...
			// 정상적으로 검색이 완료됐으면 
			 if (status === daum.maps.services.Status.OK) {
					...
					var infowindow = new daum.maps.InfoWindow({
						content: '<div style="width:150px;text-align:center;padding:6px 0;"><?=$row[curri_title]?></div>'
					});
