...
if($num2 >= $max_cnt){
	error_go("등록은 ".$max_cnt." 개 까지만 가능합니다.","curri_list.php?".$total_param);
} 
?>
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<form name="frm" id="frm" action="curri_write_action.php" target="_fra_admin" method="post"  enctype="multipart/form-data">
					...
					<tr>
						<th scope="row">이미지</th>
						<td colspan="3">
							<input type="file" style="width:400px;" required="yes" message="이미지" name="photo1_0"> (최적화 사이즈 : 가로 500, 세로 354 픽셀)
						</td>
					</tr>
					<tr>
						<th scope="row">설명글</th>
					...
					<!--<tr>
						...
						<td colspan="3">
							<input type="file" style="width:400px;" required="yes" message="오버 이미지" name="photo_0"> (최적화 사이즈 : 가로 344 픽셀, 세로 526 픽셀)
						</td>
					</tr>-->
					</table>
					...
					?>
						...
						<tr>
							...
							<td colspan="3">
								<input type="text" style="width:60%;" id="lecture_title_<?=$i_file?>" name="lecture_title_<?=$i_file?>" required="yes" message="소주제 제목" value="<?=$row_file['lecture_title']?>">
							</td>
						</tr>
						<?
						...
						<tr>
							...
							<td colspan="3">
								...
								<input type="file" style="width:400px;" required="yes" message="섬네일 이미지" name="photo2_<?=$i_file?>_<?=$i_file_lect?>"> (최적화 사이즈 : 가로 240, 세로 170 픽셀)
						...
						<tr>
							...
							<td colspan="3">
								...
								<input type="file" style="width:400px;" required="yes" message="음원" name="photo3_<?=$i_file?>_<?=$i_file_lect?>">
								<?if($row_file_lect['file_chg']){?>
									<br>기존파일 : <a href="/pro_inc/download_file.php?nm=<?=$row_file_lect['file_chg']?>&on=<?=$row_file_lect['file_org']?>&dir=curri_lecture_info"><?=$row_file_lect['file_org']?></a>
...
<!--
	...
	function uplocurri_photo(upnum){
		$("#btnUplocurri_"+upnum+"").click();
	}
	function uplocurri_file(upnum){
		$("#uplocurri_"+upnum+"").click();
	}
		...
		// 파일삭제 1 확인
			function file_del_1(){
				...
				if ( (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1) ){
					...
					$("#uplocurri_0").replaceWith( $("#uplocurri_0").clone(true) );
				} else {
			...
			function file_del_2(){
				...
				if ( (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1) ){
					 // ie 일때 input[type=file] init.
					$("#uplocurri_1").replaceWith( $("#uplocurri_1").clone(true) );
				} else {
					//other browser 일때 input[type=file] init.
					$("#uplocurri_1").val("");
				}
	...
	$(document).ready(function(){
		//get_data("mypage_host_order_inner_cal.php","inner_cal_area","");
	});

function set_cal_sdate(sdate){
	$("#s_date").val(sdate);
}
