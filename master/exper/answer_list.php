...
if($field == "exp_title"){
	$where .= " and exp_info_idx in (select idx from exp_info where 1 and exp_title like '%".$keyword."%')";
} elseif($field == "member"){
...
$query = "select *,(select exp_title from exp_info where 1 and idx=exp_info_regist.exp_info_idx) as exp_title,(select file_chg from exp_info where 1 and idx=exp_info_regist.exp_info_idx) as exp_file_chg,(select user_name from member_info where 1 and idx=exp_info_regist.member_idx) as user_name FROM exp_info_regist where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;

//echo "<br><br>쿼리 = ".$query."<br><Br>";

$result = mysqli_query($gconnet,$query);

$query_cnt = "select idx FROM exp_info_regist where 1 ".$where;
...
<!--
	...
	function go_view(no){
		location.href = "answer_view.php?idx="+no+"&<?=$total_param?>";
	}
	...
	function go_mile_pop(midx){
		window.open("../member/member_point_history.php?member_idx="+midx+"&point_sect=refund","mileview", "top=100,left=100,scrollbars=yes,resizable=no,width=1080,height=600");
	}
		...
		<div class="container clearfix">
						...
						<tr>
							...
							<td colspan="5">
								<select name="field" size="1" style="vertical-align:middle;width:20%;">
									...
									<option value="member" <?=$field=="member"?"selected":""?>>신청자</option>
								</select>
			...
			<table class="search_list">
				<thead>
					<tr>
						<th width="5%">번호</th>
						<th width="10%">체험이미지</th>
						<th width="15%">체험제목</th>
						<th width="10%">신청자명</th>
						<th width="10%">신청자 연락처</th>
						<th width="15%">신청자 이메일</th>
				...
				<? if($num==0) { ?>
					<tr>
						<td colspan="12" height="40"><strong>신청 내역이 없습니다.</strong></td>
					</tr>
				<? } ?>
				...
				<tr>
					...
					<?if($row[exp_file_chg]){?>
						<img src="<?=get_exp_image($row[exp_info_idx],"file_chg")?>" style="max-width:90%;">
					<?}?>
					</a></td>
					<td><?=$row['exp_title']?></td>
					<td><?=$row['user_name']?></td>
					<td><?=$row['cell']?></td>
					<td><?=$row['email']?></td>
					<td>[<?=$row['zipcode']?>] <?=$row['addr']?> <?=$row['addr2']?></td>
