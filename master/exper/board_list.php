...
if($bbs_code){
	$where .= " and a.bbs_code = '".$bbs_code."' and is_del='N'"; // 선택한 게시판에 해당하는 내용만 추출한다
} elseif($s_cate_code) {
		...
		<div class="container clearfix">
			...
			?>
						...
						<tr>
							...
							<td><a href="javascript:go_view('<?=$row[idx]?>','<?=$row['bbs_code']?>');"><?=$row[exp_title]?></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>','<?=$row['bbs_code']?>');"><?=$exp_cate_name1?></td>
							<!--<td><iframe  src="https://www.youtube.com/embed/<?=str_replace("https://youtu.be/","",$row[exp_url])?>" style="width:90%;" frameborder="0" allowfullscreen></iframe></td>-->
