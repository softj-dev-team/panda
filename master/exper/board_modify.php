...
if($orgin_idx){
	$sql = "SELECT *,(select file_chg from exp_info where 1 and idx=a.product_idx) as exp_file_chg,(select exp_title from exp_info where 1 and idx=a.product_idx) as exp_title,(select cate_code1 from exp_info where 1 and idx=a.product_idx) as cate_code1,(select member_idx from exp_info where 1 and idx=a.product_idx) as exp_member_idx FROM board_content a where 1 and idx = '".$idx."' ";
} else {
	$sql = "SELECT *,(select file_chg from exp_info where 1 and idx=a.product_idx) as exp_file_chg,(select exp_title from exp_info where 1 and idx=a.product_idx) as exp_title,(select cate_code1 from exp_info where 1 and idx=a.product_idx) as cate_code1,(select member_idx from exp_info where 1 and idx=a.product_idx) as exp_member_idx FROM board_content a where 1 and idx = '".$idx."' and bbs_code='".$bbs_code."'";
}
...
<div id="wrap" class="skin_type01">
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			...
			<input type="hidden" name="pro_name" value="<?=$row[exp_title]?>"/>
			...
			<table class="t_view">
				...
				<?}?>
					...
					<tr>
						<th >체험선택</th>
						<td colspan="3"><span id="pro_name_txt"><?if($row[exp_file_chg]){?><img src='<?=get_exp_image($row[product_idx],"file_chg")?>' border='0'><?}?>&nbsp;<?=$row[exp_title]?></span>&nbsp;<a href="javascript:main_product_pop();" class="btn_green">체험찾기</a></td>
					</tr>
					<tr>
						<th >체험평점</th>
						<td colspan="3">
					...
					<tr>
						...
						<td >
							<select name="bbs_sect" size="1" style="vertical-align:middle;" required="yes"  message="게시판 분류">
								...
								<option value="ad" <?=$row['bbs_sect']=="ad"?"selected":""?>>체험</option>
							</select>
						</td>-->
					</tr>
			<?}?>
