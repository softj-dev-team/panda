...
$where .= " and set_code='exper' and cate_level = '1' and is_del = 'N' "; // 체험 대카테고리만 우선 출력
...
<!--
	...
	function go_delete_2(idx,catecode1,catecode2){
		if(confirm('카테고리를 삭제하시면 복구가 불가능하며, 사이트 운영에 버그가 발생할수도 있습니다. \n\n그래도 삭제 하시겠습니까?')){
			if(confirm('중카테고리 카테고리를 삭제하시면 소속된 소카테고리 카테고리가 모두 삭제됩니다. \n\n삭제된 내용은 복구되지 않습니다. 정말로 삭제 하시겠습니까?')){	
					...
					<!-- 리스트 시작 -->
						...
						<table class="search_list">
							...
							<form method="post" action="cate_write_action.php" name="frm_1" target="_fra_admin" id="frm_1" enctype="multipart/form-data">
								...
								<input type="hidden" name="cate_level" value="1"> <!-- 대카테고리 -->
								<tr>
									<td><input type="text" style="width:90%;" name="cate_name1" required="yes" message="카테고리명" value=""></td>
						...
						<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">등록된 카테고리가 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
							for ($i=0; $i<mysqli_num_rows($result); $i++){ // 대카테고리 루프 시작
						...
						?>
							<form name="frm_cate1_<?=$i?>" method="post" action="cate_modify_action.php"  target="_fra_admin" enctype="multipart/form-data">
								...
								<tr>
									...
									<td style="text-align:left;padding-left:10px;"><input type="text" style="width:90%;" name="cate_name1" required="yes" message="카테고리명" value="<?=$row[cate_name1]?>"></td>
							...
							<td colspan="7">
								...
								<tbody>
									<form method="post" action="cate_write_action.php" name="frm2_insert_<?=$i?>" target="_fra_admin" id="frm2_insert_<?=$i?>" enctype="multipart/form-data">
										...
										<input type="hidden" name="field" value="<?=$field?>">
										<input type="hidden" name="keyword" value="<?=$keyword?>">
										<input type="hidden" name="cate_code1" value="<?=$row[cate_code1]?>"> <!-- 대카테고리 코드 -->
										<input type="hidden" name="cate_name1" value="<?=$row[cate_name1]?>"> <!-- 대카테고리 명 -->
						...
						<?if($v_step > 1 && $row[cate_code1] == $cate_code1){ // 중카테고리 리스트 시작 ?>
							...
							?>
								<tr>
									<td colspan="7">해당하는 하위카테고리가 없습니다.</td>		
								</tr>
							<?
								}
							...
							?>
									...
									<form name="frm_cate2_<?=$cate2_i?>" method="post" action="cate_modify_action.php"  target="_fra_admin" enctype="multipart/form-data">
										...
										<input type="hidden" name="cate_code1" value="<?=$cate_code1?>"> <!-- 대카테고리 코드-->
										<input type="hidden" name="bmenu" value="<?=$bmenu?>">
										<input type="hidden" name="smenu" value="<?=$smenu?>">
										<input type="hidden" name="v_step" value="<?=$v_step?>">
										<input type="hidden" name="pageNo" value="<?=$pageNo?>">
										...
										<tr>
											...
											<td style="text-align:left;padding-left:10px;"><input type="text" style="width:90%;" name="cate_name2" required="yes" message="중카테고리 명" value="<?=$cate2_row[cate_name2]?>">
											<br><br>
								...
								?>
									<input type="checkbox" name="pro_cate1[]" id="cate1_<?=$row1[common_cate_code]?>" required="no"  message="서브카테고리" value="<?=$row1[common_cate_code]?>" <?=$check_sect1?>> <?=$row1[common_cate_name]?> &nbsp; 
								<?}?>
											</td>
											<td>
											<!--<?if($cate2_row['file_c']){?>
