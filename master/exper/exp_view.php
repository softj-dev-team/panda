				...
				<div class="write">
					...
					<table>
						...
						<tr>
							...
							<td colspan="3">
							<?=$text_color?> <!--* 이미지 위에 오버랩되는 텍스트의 색상을 설정합니다.-->
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="3">
							<?if($row[file_chg2]){?>
								<img src="<?=get_exp_image($row[idx],"file_chg2")?>">
							<?}?>
						...
						<tr>
							...
							<td colspan="3">
							<?if($row[file_chg3]){?>
								<img src="<?=get_exp_image($row[idx],"file_chg3")?>">
							<?}?>
						...
						<tr>
							...
							<td colspan="3">
							<?if($row[file_chg4]){?>
								<img src="<?=get_exp_image($row[idx],"file_chg4")?>">
							<?}?>
						...
						<tr>
							...
							<td colspan="3">
							<?if($row[file_chg5]){?>
								<img src="<?=get_exp_image($row[idx],"file_chg5")?>">
							<?}?>
						...
						<tr>
							<th scope="row">배송비</th>
							<td colspan="3">
								상품 배송 시 <?=number_format($row[exp_d_money])?> 원 배송비 결제
							</td>
						</tr>
					...
					<form name="frm_request" id="frm_request" action="exp_request_action.php" target="_fra_admin" method="post" enctype="multipart/form-data">
						...
						<input type="hidden" name="tafter_add_count_1" value="10"/>
						<input type="hidden" name="member_idx" value="<?=$row[member_idx]?>"/>
					<tr>
						<th scope="row">상품 옵션</th>
						<td colspan="3">
						<?
						//for($i_add=0; $i_add<mysqli_num_rows($query_add_1); $i_add++){
						for($i_add=0; $i_add<10; $i_add++){
							$row_add = mysqli_fetch_array($query_add_1);
						?>
							<?if($row_add['idx']){?><input type="hidden" name="option_idx_<?=$i_add?>" value="<?=$row_add['idx']?>" /><?}?>
							<?if($i_add > 0){?><br><br><?}?><input type="text" name="option_title_<?=$i_add?>" id="opt_title_<?=$i_add?>"  value="<?=$row_add['opt_title']?>" style="width:60%;" /><?if($row_add['idx']){?> 삭제 : <input type="checkbox" name="option_del_<?=$i_add?>" value="Y"><?}?>
						<?}?>
