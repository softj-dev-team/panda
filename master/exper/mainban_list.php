...
$where = " and main_sect = 'exp' and is_del='N'";
...
$query = "select *,(select file_chg from exp_info where 1 and idx=main_display_set.pro_idx) as exp_file_chg,(select exp_title from exp_info where 1 and idx=main_display_set.pro_idx) as exp_title,(select cate_code1 from exp_info where 1 and idx=main_display_set.pro_idx) as cate_code1 FROM main_display_set where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
	...
	<div class="sub_wrap">
		...
		<p style="text-align:right;padding-right:10px;padding-bottom:5px;"><font style="color:red;">* 정렬순서는 숫자만 입력 가능, 높은 숫자를 우선으로 정렬됨</font></p>
			<table class="search_list">
				...
				<? if($num==0) { ?>
					<tr>
						<td colspan="12" height="40"><strong>설정된 체험이 없습니다.</strong></td>
					</tr>
				<? } ?>
				...
				<input type="hidden" name="total_param" value="<?=$total_param?>"/>
					...
					<?}else{?>
						...
						<?if($row[exp_file_chg]){?>
							<img src="<?=get_exp_image($row[pro_idx],"file_chg","3")?>" style="max-width:90%;">
						<?}?>
						<!--</a>--></td>
						<td><!--<a href="javascript:go_view('<?=$row[idx]?>');">--><?=$exp_cate_name1?><!--</a>--></td>
