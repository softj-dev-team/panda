...
if($cr_s_date){ // 기간1 
	$where .= " and substring(a.sdate,1,10) >= '".$cr_s_date."' ";
}
if($cr_e_date){ // 기간2 
	$where .= " and substring(a.edate,1,10) <= '".$cr_e_date."' ";
}
...
$query = "select *,(select user_name from member_info where 1 and del_yn='N' and idx=a.admin_idx) as admin_name,(select user_id from member_info where 1 and del_yn='N' and idx=a.admin_idx) as admin_id from advertising_info a where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
$result = mysqli_query($gconnet,$query);

$query_cnt = "select a.idx from advertising_info a where 1 ".$where;
				...
				<form name="s_mem" id="s_mem" method="post" action="<?=basename($_SERVER['PHP_SELF'])?>">
					...
					<input type="hidden" name="s_order" id="s_order" value="<?=$s_order?>"/>
						...
						<tr>
							...
							<td colspan="2">
								<input type="text" autocomplete="off" readonly name="cr_s_date" id="cr_s_date" style="width:40%;" class="datepicker" value="<?=$cr_s_date?>"> ~ <input type="text" autocomplete="off" readonly name="cr_e_date" id="cr_e_date" style="width:40%;" class="datepicker" value="<?=$cr_e_date?>">
							</td>
			...
			<!-- 엑셀 출력을 위한 전송 폼 종료 -->
				...
				<form method="post" name="frm" target="_fra_admin" id="frm">
					...
					<input type="hidden" name="pageNo" value="<?=$pageNo?>" />
						...
						<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">발송한 <?=$cr_cate_str?>가 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
						...
						<tr>
							...
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$row['sdate']?> ~ <?=$row['edate']?></a></td>
...
<script>
	...
	function go_view(no){
		location.href = "advert_view.php?idx="+no+"&pageNo=<?=$pageNo?>&<?=$total_param?>";
	}
	...
	function go_regist(){
		location.href = "advert_write.php?<?=$total_param?>";
	}
