...
if($_SESSION['admin_coinc_idx'] == $row['member_idx']){ 
	...
	$sql_prev = "select idx from board_view_cnt where 1=1 and board_tbname='board_content' and board_code = '".$row['bbs_code']."' and board_idx='".$row['idx']."' and member_idx = '".$_SESSION['admin_coinc_idx']."' ";
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			...
			<? } ?>
					...
					<?}?>
					<tr>
						<th ><?if($bbs_code == "faq"){?>답 변<?}else{?>내 용<?}?></th>
						<td colspan="3">
						<?if($row['is_html'] == "Y"){?>
