...
<?
	...
	$visible_query_cnt = "select idx from product_info where 1 and is_del='N' and idx in (select target_idx from main_select_info where 1 and is_del='N' and type='topprod' and view_ok='Y') and idx in (select product_idx from product_info_sale where 1 and is_del='N' and public_ok='Y' and member_idx=product_info.member_idx)"; // 노출설정된 작품이 있는지
	$visible_result_cnt = mysqli_query($gconnet,$visible_query_cnt);
	$visible_num = mysqli_num_rows($visible_result_cnt);
?>
<body>
			...
			<!-- 엑셀 출력을 위한 전송 폼 종료 -->
					...
					<!-- 목록 옵션 종료 -->
						...
						<table class="search_list">
							...
							?>
								<form name="frm_cate1_<?=$row['idx']?>" method="post" action="main_select_top_list_action.php" target="_fra_admin" enctype="multipart/form-data">
...
<script>
	...
	function go_list(){
		location.href = "main_select_top_list.php?<?=$total_param?>";
	}

	function go_regist(){
		//location.href = "product_write.php?<?=$total_param?>";
		window.open("main_product_top.php","pro_pro_view", "top=100,left=100,scrollbars=yes,resizable=no,width=1280,height=500");
	}
