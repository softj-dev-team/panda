...
$where = " and a.del_yn='N' and a.msg_type='push'"; // 수동발송 메시지 
...
if(!empty($s_uname)){ // 성명 
	...
	for($si=0; $si<sizeof($s_uname); $si++){
		if($si == sizeof($s_uname)-1){
			...
			$where .= " b.member_idx in (select idx from member_info where 1 and del_yn='N' and user_name like '%".trim($s_uname[$si])."%')";
		} else {
			...
			$where .= " b.member_idx in (select idx from member_info where 1 and del_yn='N' and user_name like '%".trim($s_uname[$si])."%') or";
		}
	}
	$where .= ")";
}
...
//$query = "select a.*,(select user_nick from member_info where 1 and del_yn='N' and idx=b..member_idx) as user_nick,(select email from member_info where 1 and del_yn='N' and idx=b.member_idx) as user_email from send_msg a inner join send_msg_member b on a.idx=b.msg_idx where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;

$query = "select a.*,(select user_name from member_info where 1 and del_yn='N' and idx=a.admin_idx) as admin_name,(select user_id from member_info where 1 and del_yn='N' and idx=a.admin_idx) as admin_id from send_msg a where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
...
$query_cnt = "select a.idx from send_msg a where 1 ".$where;
					...
					<input type="hidden" name="pageNo" value="<?=$pageNo?>" />
						...
						<?
							for ($i=0; $i<mysqli_num_rows($result); $i++){
								...
								$msg_content = string_cut2(stripslashes(strip_tags($row['msg_content'])),100); 
						?>
						<tr>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$listnum?></a></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$arr_push_type[$row['msg_cate']]?></a></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$msg_content?></a></td>
...
function go_view(no){
	location.href = "msg_send_view.php?idx="+no+"&pageNo=<?=$pageNo?>&<?=$total_param?>";
}
</script>
