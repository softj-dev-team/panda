...
$where .= " and set_code='memcat' and cate_level = '1' and is_del = 'N' "; // 상품 대관심사만 우선 출력
...
<!--
	...
	function go_delete_2(idx,catecode1,catecode2){
		if(confirm('관심사를 삭제하시면 복구가 불가능하며, 사이트 운영에 버그가 발생할수도 있습니다. \n\n그래도 삭제 하시겠습니까?')){
			if(confirm('중관심사 관심사를 삭제하시면 소속된 소관심사 관심사가 모두 삭제됩니다. \n\n삭제된 내용은 복구되지 않습니다. 정말로 삭제 하시겠습니까?')){	
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list">
					...
					<!-- 검색창 시작 -->
					<form name="s_mem" method="post" action="cate_list.php">
					...
					<!-- 리스트 시작 -->
						...
						<table class="search_list">
							...
							<form method="post" action="cate_write_action.php" name="frm_1" target="_fra_admin" id="frm_1" enctype="multipart/form-data">
								...
								<input type="hidden" name="cate_level" value="1"> <!-- 대관심사 -->
								<tr>
									<td><input type="text" style="width:90%;" name="cate_name1" required="yes" message="관심사명" value=""></td>
									<td><input type="file" name="file1" style="width:50%;" required="yes"  message="관심사 아이콘"/></td>
						...
						<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">등록된 관심사가 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
							for ($i=0; $i<mysqli_num_rows($result); $i++){ // 대관심사 루프 시작
						...
						?>
							<form name="frm_cate1_<?=$i?>" method="post" action="cate_modify_action.php"  target="_fra_admin" enctype="multipart/form-data">
								...
								<input type="hidden" name="cate_level" value="<?=$row[cate_level]?>"> <!-- 관심사 구분-->
								<input type="hidden" name="bmenu" value="<?=$bmenu?>">
								<input type="hidden" name="smenu" value="<?=$smenu?>">
								<input type="hidden" name="v_step" value="<?=$v_step?>">
								<input type="hidden" name="pageNo" value="<?=$pageNo?>">
								...
								<tr>
									...
									<td style="text-align:left;padding-left:10px;"><input type="text" style="width:90%;" name="cate_name1" required="yes" message="관심사명" value="<?=$row[cate_name1]?>"></td>
									<td style="padding-top:10px;padding-bottom:10px;">
										<?if($row['file_c']){?>
											<a href="/pro_inc/download_file.php?nm=<?=$row['file_c']?>&on=<?=$row['file_o']?>&dir=cate_banner"><img src="<?=$_P_DIR_WEB_FILE?>/cate_banner/<?=$row[file_c]?>" style="max-width:90%;"></a>
							...
							<td colspan="7">
								...
								<tbody>
									<form method="post" action="cate_write_action.php" name="frm2_insert_<?=$i?>" target="_fra_admin" id="frm2_insert_<?=$i?>" enctype="multipart/form-data">
										...
										<input type="hidden" name="field" value="<?=$field?>">
										<input type="hidden" name="keyword" value="<?=$keyword?>">
										<input type="hidden" name="cate_code1" value="<?=$row[cate_code1]?>"> <!-- 대관심사 코드 -->
										<input type="hidden" name="cate_name1" value="<?=$row[cate_name1]?>"> <!-- 대관심사 명 -->
										<input type="hidden" name="cate_level" value="2"> <!-- 중관심사 -->
										<tr>
											<td ><input type="text" style="width:90%;" name="cate_name2" required="yes" message="중관심사 명" value=""></td>
											<td><input type="file" name="file1" style="width:50%;" required="no"  message="메인 이미지"/>
												<br> <font style="color:blue;">이미지를 등록하시면 쇼핑몰 메인화면 관심사에 설정됩니다</font>
											</td>
											<td><input type="text" style="width:20%;" name="cate_align" required="yes" message="정렬순서" is_num="yes" value=""> 숫자만 입력, 높은숫자 우선순위</td>
						...
						<?if($v_step > 1 && $row[cate_code1] == $cate_code1){ // 중관심사 리스트 시작 ?>
							<?
							...
							?>
								<tr>
									<td colspan="7">해당하는 하위관심사가 없습니다.</td>		
								</tr>
							<?
								}

								for($cate2_i=0; $cate2_i<$cate2_cnt; $cate2_i++){ // 중관심사 루프 시작
							...
							?>
									...
									<form name="frm_cate2_<?=$cate2_i?>" method="post" action="cate_modify_action.php"  target="_fra_admin" enctype="multipart/form-data">
										...
										<input type="hidden" name="cate_level" value="<?=$cate2_row[cate_level]?>"> <!-- 관심사 구분-->
										<input type="hidden" name="cate_code1" value="<?=$cate_code1?>"> <!-- 대관심사 코드-->
										<input type="hidden" name="bmenu" value="<?=$bmenu?>">
										<input type="hidden" name="smenu" value="<?=$smenu?>">
										<input type="hidden" name="v_step" value="<?=$v_step?>">
										<input type="hidden" name="pageNo" value="<?=$pageNo?>">
										...
										<tr>
											...
											<td style="text-align:left;padding-left:10px;"><input type="text" style="width:90%;" name="cate_name2" required="yes" message="중관심사 명" value="<?=$cate2_row[cate_name2]?>">
											<br><br>
								...
								?>
									<input type="checkbox" name="pro_cate1[]" id="cate1_<?=$row1[common_cate_code]?>" required="no"  message="서브관심사" value="<?=$row1[common_cate_code]?>" <?=$check_sect1?>> <?=$row1[common_cate_name]?> &nbsp; 
								<?}?>
											</td>
											<td>
											<!--<?if($cate2_row['file_c']){?>
