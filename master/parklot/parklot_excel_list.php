...
$where = " and memout_yn != 'Y' and memout_yn != 'S' and is_del='N'";
...
if($v_sect){
	$where .= " and parklot_type = '".$v_sect."'";
}

if($s_gubun){
	if($s_gubun == "NOR2"){
		$where .= " and ipin_code = 'cell' "; // 휴대폰 인증주차장
	} else {
		$where .= " and parklot_gubun = '".$s_gubun."' ";
	}
}
...
$query = "select *,(select s_date from hotelship_auth where 1 and is_del='N' and parklot_idx=a.idx order by idx desc limit 0,1) as s_date,(select e_date from hotelship_auth where 1 and is_del='N' and parklot_idx=a.idx order by idx desc limit 0,1) as e_date from parklot_info a where 1 ".$where.$order_by;
//echo $query; //exit;
$result = mysqli_query($gconnet,$query);

if($s_sect2){
	$b_name_sql = "select com_name from parklot_info where 1 and idx='".$s_sect2."'";
