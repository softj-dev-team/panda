...
/*if($s_gubun == "NOR"){
	...
	$parklot_sect_str = "VVIP 멤버십";
}*/

$sql = "SELECT * FROM parklot_info a where 1 and idx = '".$idx."' and is_del='N'";
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list_tit">
					<h3><?=$parklot_str?> 정보 수정</h3>
				</div>
				<div class="write">
				<form name="frm" action="parklot_modify_action.php" target="_fra_admin" method="post"  enctype="multipart/form-data">
					<input type="hidden" name="idx" id="parklot_idx" value="<?=$idx?>"/>
						...
						<tr class="address">
								...
								<p>
									<select name="sido" id="sido" style="vertical-align:middle;width:20%;" required="yes" message="지역" onchange="area_sel_1(this)">
										...
										?>
											<option value="<?=$row1['bjd_code']?>" <?=$row['sido']==$row1['bjd_code']?"selected":""?>><?=$row1['k_name']?></option>
										<?}?>
									</select>
									...
									<?if($row['sido']){?>
										<?
										$sect1_sql = "select bjd_code,k_name from code_bjd where 1 and filter='SGG' and del_yn='N' and pre_code='".$row['sido']."' order by k_name asc";
										$sect1_result = mysqli_query($gconnet,$sect1_sql);
											for ($i=0; $i<mysqli_num_rows($sect1_result); $i++){
												$row1 = mysqli_fetch_array($sect1_result);
										?>
											<option value="<?=$row1['bjd_code']?>" <?=$row['gugun']==$row1['bjd_code']?"selected":""?>><?=$row1['k_name']?></option>
										<?}?>
									<?}?>
									</select>
								</p>
						...
						<tr>
							...
							<?
								for($file_i=0; $file_i<$cnt_file; $file_i++){
									...
									$pub_time_s_arr = explode(":",$row_file['pub_time_s']);
									$pub_time_s_h = $pub_time_s_arr[0];
									$pub_time_s_m = $pub_time_s_arr[1];
									$pub_time_e_arr = explode(":",$row_file['pub_time_e']);
									$pub_time_e_h = $pub_time_e_arr[0];
									$pub_time_e_m = $pub_time_e_arr[1];
							?>
								...
								<div <?if($file_i > 0){?>style="margin-top:10px;"<?}?>> <?=$inc_pubyoil_arr[$file_k]?> : 
									...
									&nbsp; 공유시 체크 : <input type="checkbox" name="pub_time_yn_<?=$file_k?>" id="pub_time_yn_<?=$file_k?>" value="Y" <?=$row_file['pub_time_yn']=="Y"?"checked":""?>>
								</div>
							<?}?>
							</td>
						</tr>
...
function go_list(){
		location.href = "parklot_view.php?idx=<?=$idx?>&<?=$total_param?>";
}
