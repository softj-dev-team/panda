...
<?
	...
	$prev_sql = "select idx,user_level from parklot_info where 1=1 and idx = '".$idx."' and memout_yn = 'N' ";
	$prev_query = mysqli_query($gconnet,$prev_sql);
	$prev_cnt = mysqli_num_rows($prev_query);
	
	if($prev_cnt == 0){
		error_frame("설정을 변경할 주차장이 존재하지 않습니다.");
		exit;
	}
	...
	/*if($prev_row['user_level'] != $user_level){ // 주차장 레벨을 변경하고자 할때
		
		$set_section = "level";
		
		$query_change_level = " insert into parklot_set_change set "; 
		$query_change_level .= " parklot_idx = '".$idx."', ";
	...
	parent.location.href =  "parklot_view.php?idx=<?=$idx?>&<?=$total_param?>";
	//-->
	</SCRIPT>
	<?}else{?>
	<SCRIPT LANGUAGE="JavaScript">
