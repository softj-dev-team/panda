...
<?
	...
	/* 회원 사진 삭제
	$file_sql = "select file_chg from member_info where 1=1 and idx = '".$idx."' ";
	...
	$file2_sql = "select file_chg,file_chg2,file_chg3,file_chg4 from member_info_detail where 1 and member_idx = '".$idx."' ";
	...
	// 회원 정보 삭제 
	$query = "update member_info set del_yn='Y'"; 
	$query .= " where idx = '".$idx."' ";
	$result = mysqli_query($gconnet,$query);

	/* 상세 정보 삭제 
	$query_dt = " delete from member_info_detail "; 
	$query_dt .= " where member_idx = '".$idx."' ";
	$result_dt = mysqli_query($gconnet,$query_dt);
	
	// 포인트 적립/누적 히스토리 삭제 
	$query_point = " delete from member_point "; 
	$query_point .= " where member_idx = '".$idx."' ";
	...
	<!--
	alert('<?=$member_sect_str?> 정보삭제가 정상적으로 완료 되었습니다.');
