...
$where = " and memout_yn not in ('Y','S') and del_yn='N' and member_type in ('PAT')";
...
if ($field && $keyword){
	if($field == "com_name"){
		$where .= " and idx in (select member_idx from member_info_company where 1 and is_del='N' and ".$field." like '%".$keyword."%')";
	} else {
...
$query = "select *,(select com_name from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_name,(select com_homep from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_homep,(select com_num_1 from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_num_1,(select com_num_2 from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_num_2,(select com_uptae from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_uptae,(select com_jong from member_info_company where 1 and is_del='N' and member_idx=a.idx order by idx desc limit 0,1) as com_jong from member_info a where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
...
function go_tot_del() {
	...
	if(check) {
		if(confirm('선택하신 가맹점을 삭제 하시겠습니까?')){
			if(confirm('삭제하신 가맹점정보는 복구할 수 없습니다. 정말 삭제 하시겠습니까?')){
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list_tit">
					<h3>가맹점 리스트</h3>
				<?//if($s_gubun == "NOR"){?>
					<button class="btn_add" onclick="go_regist();" style="width:15%;"><span>가맹점 등록</span></button>
				<?//}?>
				</div>
				<!-- 네비게이션 종료 -->
				<div class="list">
			...
			<!-- 엑셀 출력을 위한 전송 폼 종료 -->
				...
				<form method="post" name="frm" target="_fra_admin" id="frm">
					...
					<input type="hidden" name="total_param" value="<?=$total_param?>"/>
						...
						?>
						<tr>
							<td><input type="checkbox" name="member_idx[]" id="member_idx[]" value="<?=$row["idx"]?>" required="yes"  message="가맹점"/></td>
							<td><?=$listnum?></td>
							...
							<td><a href="javascript:go_view('<?=$row['idx']?>');"><?=$row['com_num_1']?></a></td>
							<td><a href="javascript:go_view('<?=$row['idx']?>');"><?=$row['com_num_2']?></a></td>
							<td><a href="javascript:go_view('<?=$row['idx']?>');"><?=$row['com_uptae']?></a></td>
							<td><a href="javascript:go_view('<?=$row['idx']?>');"><?=$row['com_jong']?></a></td>
