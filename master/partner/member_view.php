...
$sql = "SELECT * FROM member_info a where 1 and idx = '".$idx."' and del_yn='N'";
				...
				<div class="write">
					<p class="tit">사업자 정보</p>
						...
						<tr>
							...
							<td>
								<?=$row_company['com_chair']?>
							</td>
							<th scope="row">회사명</th>
							<td>
								<?=$row_company['com_name']?>
							</td>
						</tr>
						<tr>
							<th scope="row">개별도메인</th>
							<td colspan="3">
								<a href="https://<?=$row_company['com_homep']?>.pandasms.co.kr" target="_blank">https://<?=$row_company['com_homep']?>.pandasms.co.kr</a>
							</td>
						</tr>
						<tr>
							<th scope="row">사업자번호</th>
							<td>
								<?=$row_company['com_num_1']?>
							</td>
							<th scope="row">통신판매업신고번호</th>
							<td>
								<?=$row_company['com_num_2']?>
							</td>
						</tr>
						<tr>
							<th scope="row">업태</th>
							<td>
								<?=$row_company['com_uptae']?>
							</td>
							<th scope="row">종목</th>
							<td>
								<?=$row_company['com_jong']?>
							</td>
						</tr>
						<tr>
							<th scope="row">대표 전화번호</th>
							<td><?=$row_company['com_tel_1']?></td>
							<th scope="row">Fax</th>
							<td><?=$row_company['com_fax']?></td>
						</tr>
						<tr>
							<th scope="col">주소</th>
							<td scope="col" colspan="3">[<?=$row_company['com_zip']?>] <?=$row_company['com_addr1']?> <?=$row_company['com_addr2']?></td>
						</tr>
						<tr>
							...
							?>
									<div <?if($i_file > 0){?>style="margin-top:10px;"<?}?>>
										<a href="/pro_inc/download_file.php?nm=<?=$row_file['file_chg']?>&on=<?=$row_file['file_org']?>&dir=partner"><?=$row_file['file_org']?></a>
									</div>
							<?}?>
							</td>
						</tr>
					...
					<colgroup>
						...
						<form name="set_frm" id="set_frm" action="member_view_action.php" target="_fra_admin" method="post" >
							...
							<td>
								<select name="login_ok" required="yes" message="가맹점 상태" size="1" style="vertical-align:middle;" >
							...
							<td>
								<select name="com_status" required="yes" message="승인여부" size="1" style="vertical-align:middle;" >
									<option value="">선택하세요</option>
									<option value="1" <?=$row_company['com_status']=="1"?"selected":""?>>신청</option>
									<option value="2" <?=$row_company['com_status']=="2"?"selected":""?>>승인</option>
									<option value="3" <?=$row_company['com_status']=="3"?"selected":""?>>거절</option>
								</select>
							</td>
							</tr>
