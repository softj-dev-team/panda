...
$query_cnt = "select idx from membership_auth where 1 ".$where;
...
$query = "select * from membership_auth where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
...
?>
				...
				<form method="post" name="frm" target="_fra_admin" id="frm">
					...
					<table class="search_list" style="margin-top:10px;">
						...
						?>
							<tr>
								<td><input type="checkbox" name="membership_idx[]" id="membership_idx[]" value="<?=$row["idx"]?>" required="yes"  message="결제정보"/></td>
								<td><?=$listnum?></td>
								<td><?=$row['wdate']?></td>
								<td><?=get_code_value("cate_name1","cate_code1",$row['payment_type'])?></td>
								...
								<td>
								<?if($row['pay_sect_1']){?>
									<?=get_payment_method($row['pay_sect_1'])?>
								<?}else{?>
									-
								<?}?>
								</td>
								<td><?=get_pay_status($row['pay_status'])?></td>
							</tr>
						<?}?>
						...
						<div style="text-align:right;margin-top:10px;padding-right:10px;">
							...
							<a href="javascript:go_tot_stop();" class="btn_green">선택정지</a>
							<a href="javascript:membership_regist();" class="btn_blue">멤버쉽 추가</a>
						</div>

					<div class="pagination mt0">
					<?
						$target_link = "membership_list.php";
						$target_id = "membership_list_area";
...
function CheckAll(){                
	...
	var chk = document.getElementsByName("membership_idx[]")                 //체크박스의 name값
...
function go_tot_del() {
	...
	if(check) {
		if(confirm('선택하신 결제정보를 삭제 하시겠습니까?')){
			frm.action = "membership_list_action_del.php";
			frm.submit();
		}
	} else {
		false;
...
function go_tot_stop() {
	var check = chkFrm('frm');
	if(check) {
		if(confirm('선택하신 결제정보를 정지 하시겠습니까?')){
			frm.action = "membership_list_action_stop.php";
			frm.submit();
		}
	} else {
		false;
