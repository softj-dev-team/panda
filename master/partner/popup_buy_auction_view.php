...
<?
	...
	// 오퍼이력 쿼리 시작 
		...
		$where = " and is_del='N' and sale_idx='".$row_basic['sale_idx']."'";
	...
	$query_last = "select *,(select user_nick from member_info where 1 and del_yn='N' and idx=product_info_sale_auction.member_idx) as user_nick from product_info_sale_auction where 1 and auction_yn='Y' and is_del='N' and sale_idx='".$row_basic['sale_idx']."'";
			  ...
			  <!-- 기본정보 시작 -->
				<div class="write">
					<table>
							...
							<tr>
								<td colspan="4" style="background-color:#ffffff;">[<?=$row_basic['product_title']?>] 구매내역 보기</td>
							</tr>
							<tr>
								<th scope="row">구매자</th>
								<td style="background-color:#ffffff;"><?=$row_basic['user_buy']?></td>
				...
				<div class="list">
					<div class="search_wrap">
						<table class="search_list">
							...
							<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">경매참여 내역이 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
			...
			<?} else {?>
				...
				<p class="tit">낙찰정보</p>
					<table>
							...
							<tr>
								<th scope="row" colspan="4">낙찰자가 없습니다.</th>
							</tr>
						</table>
					</div>	
			<? } ?>
