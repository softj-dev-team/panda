...
function fn_Bill(trd_no, vat) {
	...
	<div class="inner">
		...
		<table class="t_list">
				...
				<tbody>
				<?	
					$product_sql = "select *,(select file_c from product_info where 1 and idx=order_product.product_idx) as file_c
			...
			<colgroup>
				...
				<tr>
					<th>결제일시</th>
					<td colspan="3"><?=get_local_datetime($row['order_date'], "Y-m-d H:i:s")?></td>
				</tr>
				...
				<?if($row[orderstat] == "can" || $row[orderstat] == "can1"){?>
					...
					<tr>
						<th>결제취소 일시</th>
						<td colspan="3"><?=get_local_datetime($row['cancel_date'], "Y-m-d H:i:s")?></td>
					</tr>
					<tr>
						<th>취소유형</th>
				...
				<?}elseif($row[orderstat] == "reing" ||$row[orderstat] == "recom"){?>
					...
					<tr>
						<th>반품처리 일시</th>
						<td colspan="3"><?=get_local_datetime($row['repay_done_date'], "Y-m-d H:i:s")?></td>
					</tr>
			...
			<br><br>
			<p><font style="color:blue;"><b>결제취소</b></font></p> 
		...
		<?if(($row[orderstat] == "com" || $row[orderstat] == "reing") && $row["delvstat"] != "d_deny"){?>
			...
			?>
			<br><br>
			<p><font style="color:blue;"><b>배송정보</b></font> &nbsp;&nbsp;<font style='color:red;'>주문상품 배송시 반드시 배송상태를 배송중 혹은 배송완료 로 설정해 주셔야 배송중 고객의 주문취소를 막을수 있습니다.</font></p>
			<form name="frm2" action="order_delvset_action.php" target="_fra_admin" method="post" >
			<input type="hidden" name="order_num" value="<?=$order_num?>"/>
			<input type="hidden" name="total_param" value="<?=$total_param?>"/>
			<table class="t_view">
				<colgroup>
				...
				<tr>
					<th >배송상태</th>
					<td>
						<b><font style="color:blue;"><?=$delv_stat_str?></font></b>
					<?
					if($row["delvlink"]){
					?>
						<!--&nbsp;<a href="#" onclick="window.open('<?=$row["delvlink"]?>', '', 'width=500,height=400,resizable=1');" class="btn_blue2">배송추적</a>-->
						&nbsp;<a href="<?=$row["delvlink"]?>" target="_blank" class="btn_blue2">배송추적</a>
					<?
					}
					?>
					</td>
				...
				<tr>
					<th>배송업체</th>
					<td><?=get_delivery_company($row[delvcom])?></td>
				</tr>
				<tr>
					<th>배송번호</th>
					<td><?=$row[delvnum]?></td>
				...
				<tr>
					<th >배송상태</th>
					<td>
					<select name="delvstat" size="1" style="vertical-align:middle;" onchange="delvstat_sel(this)" required="yes" message="배송상태">
						<option value="">배송상태 설정</option>
						<?if(!$row[delvstat] || $row[delvstat] == "d_pre"){?>
							<option value="d_pre" <?=$row["delvstat"]=="d_pre"?"selected":""?>>배송준비</option>
						<?}?>
						<option value="d_ing" <?if($row["delvstat"] == "d_ing") echo "selected";?>>배송중</option>
						<option value="d_com" <?if($row["delvstat"] == "d_com") echo "selected";?>>배송완료</option>
						<!--<option value="d_conf">고객수령확인</option>-->
					</select> &nbsp; <a href="#" onclick="go_submit2();" class="btn_blue2">상태설정</a>
				<!--&nbsp;<a href="javascript:k_Popup('/mypage/delv_stat.php?order_num=<?=$row[order_num]?>','sview','top=100,left=100,scrollbars=no,resizable=yes,width=620,height=345');" class="btn_blue2">배송조회			</a> -->
					</td>
				</tr>
				<tr id="delv_txt_0" style="display:<?=($row[delvstat]=="d_pre" || $row["delvstat"] == "")?"none":""?>;">
					<th>배송등록자</th>
					<td><input type="text" name="delvname" value="<?if($row["delvname"]) echo $row["delvname"]; else echo htmlspecialchars($session_admin_name);?>" size=17 maxlength=30 style="width:100px;" required="no"  message="배송등록자" /></td>
				</tr>
				<tr id="delv_txt_1" style="display:<?=($row[delvstat]=="d_pre" || $row["delvstat"] == "")?"none":""?>;">
					<th>배송업체명</th>
					<td>
					<!--<input type="text" style="width:80%;" name="delvcom" required="no"  message="배송업체명" value="<?=$row[delvcom]?>">-->
						<select style="width:40%; height:24px;" name="delvcom" id="delvcom" required="no" message="배송업체명">
							<option value=''>선택하십시오</option>
							<option value='kyoungdong' <?if($row["delvcom"] == "kyoungdong") echo "selected";?>>경동택배</option>
							<option value='ajutb' <?if($row["delvcom"] == "ajutb") echo "selected";?>>아주택배</option>
							<option value='ktlogistics' <?if($row["delvcom"] == "ktlogistics") echo "selected";?>>KT로지스</option>
							<option value='hyundai' <?if($row["delvcom"] == "hyundai") echo "selected";?>>현대택배</option>
							<option value='cjgls' <?if($row["delvcom"] == "cjgls") echo "selected";?>>CJ대한통운</option>
							<option value='hanjin' <?if($row["delvcom"] == "hanjin") echo "selected";?>>한진택배</option>
							<option value='tranet' <?if($row["delvcom"] == "tranet") echo "selected";?>>트라넷</option>
							<option value='Hanaro' <?if($row["delvcom"] == "Hanaro") echo "selected";?>>하나로택배</option>
							<option value='Sagawa' <?if($row["delvcom"] == "Sagawa") echo "selected";?>>사가와익스프레스</option>
							<option value='sedex' <?if($row["delvcom"] == "sedex") echo "selected";?>>SEDEX</option>
							<option value='kgbls' <?if($row["delvcom"] == "kgbls") echo "selected";?>>KGB택배</option>
							<option value='kgb' <?if($row["delvcom"] == "kgb") echo "selected";?>>로젠택배</option>
							<option value='yellow' <?if($row["delvcom"] == "yellow") echo "selected";?>>옐로우캡</option>
							<option value='hth' <?if($row["delvcom"] == "hth") echo "selected";?>>삼성HTH</option>
							<option value='dongbu' <?if($row["delvcom"] == "dongbu") echo "selected";?>>KG로지스(구 동부택배포함)</option>
							<option value='EPOST' <?if($row["delvcom"] == "EPOST") echo "selected";?>>우체국택배</option>
							<option value='registpost' <?if($row["delvcom"] == "registpost") echo "selected";?>>우편등기</option>
							<option value='9999' <?if($row["delvcom"] == "9999") echo "selected";?>>기타택배</option>
						</select>
					</td>
				</tr>
				<tr id="delv_txt_2" style="display:<?=($row[delvstat]=="d_pre" || $row["delvstat"] == "")?"none":""?>;">
					<th>배송번호</th>
					<td>
						<input type="text" style="width:80%;" name="delvnum" required="no"  message="배송번호" value="<?=$row[delvnum]?>">
					</td>
				</tr>
				<tr id="delv_txt_3" style="display:<?=($row["delvstat"] == "d_pre" || $row[delvstat]=="")?"none":""?>;">
					<th>배송추적 확인링크</th>
					<td>
						<input type="text" style="width:80%;" name="delvlink" required="no"  message="배송상태 확인링크" value="<?=$row[delvlink]?>"> 
					</td>
				</tr>
				<?}?>	
			</table>
