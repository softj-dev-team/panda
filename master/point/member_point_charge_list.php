...
$orderstat = sqlfilter($_REQUEST['orderstat']); // 대기,승인,반려
...
################## 파라미터 조합 #####################
$total_param = 'bmenu=' . $bmenu . '&smenu=' . $smenu . '&field=' . $field . '&keyword=' . $keyword . '&v_sect=' . $v_sect . '&s_gubun=' . $s_gubun . '&s_level=' . $s_level . '&s_gender=' . $s_gender . '&s_sect1=' . $s_sect1 . '&s_sect2=' . $s_sect2 . '&s_cnt=' . $s_cnt . '&s_order=' . $s_order . '&orderstat=' . $orderstat;

$where = " and a.is_del='N' and a.contents_tbname='member_point' and a.member_idx in (select idx from member_info where 1 and memout_yn not in ('Y','S') and del_yn='N' and member_type in ('GEN'))";
...
$query = "select *,(select user_id from member_info where 1 and del_yn='N' and member_type in ('GEN') and memout_yn='N' and idx=a.member_idx) as user_id_r,(select user_name from member_info where 1 and del_yn='N' and member_type in ('GEN') and memout_yn='N' and idx=a.member_idx) as user_name_r from order_member a where 1 " . $where . $order_by . " limit " . $StarRowNum . " , " . $EndRowNum;
...
<script type="text/javascript">
		...
		if (check) {
			if (confirm('선택하신 충전내역을 삭제 하시겠습니까?')) {
				if (confirm('삭제하신 충전내역은 복구할 수 없습니다. 정말 삭제 하시겠습니까?')) {
					frm.action = "../partner/member_payment_action_del.php";
					frm.submit();
				}
			}
		} else {
	...
	function go_payment_complete(order_num) {
		if (confirm('선택하신 결제신청의 상태를 승인으로 변경하시겠습니까?')) {
			if (confirm('승인 변경후에는 이전 상태로 복구할 수 없습니다. 정말 변경 하시겠습니까?')) {
				_fra_admin.location.href = "../partner/member_payment_action_paycom.php?order_num=" + order_num + "";
			}
		}
	}

	function go_payment_return(order_num) {
		if (confirm('선택하신 결제신청의 상태를 반려로 변경하시겠습니까?')) {
			if (confirm('반려 변경후에는 이전 상태로 복구할 수 없습니다. 정말 변경 하시겠습니까?')) {
				_fra_admin.location.href = "../partner/member_payment_action_return.php?order_num=" + order_num + "";
			}
		}
	}
							...
							<form name="s_mem" id="s_mem" method="post" action="<?= basename($_SERVER['PHP_SELF']) ?>">
								...
								<tr>
									...
									<td colspan="2">
										...
										<input type="radio" name="v_sect" id="v_sect_3" value="1" <?= $v_sect == "1" ? "checked" : "" ?>> 대기
										<input type="radio" name="v_sect" id="v_sect_3" value="3" <?= $v_sect == "3" ? "checked" : "" ?>> 반려
									</td>
							...
							<form method="post" name="frm" target="_fra_admin" id="frm">
								...
								<table class="search_list">
									...
									<tbody>
										<? if ($num == 0) { ?>
											<tr>
												<td colspan="12" height="40">충천내역이 없습니다.</strong></td>
											</tr>
										<? } ?>
										<?
											...
											<tr>
												...
												<td><!--<a href="javascript:go_view('<?= $row['idx'] ?>');">--><?
																												if ($row['orderstat'] == "pre") {
																													echo "대기";
																												} else if ($row['orderstat'] == "com") {
																													echo "승인";
																												} else if ($row['orderstat'] == "ret") {
																													echo "반려";
																												}
																												?><!--</a>--></td>
												<td><!--<a href="javascript:go_view('<?= $row['idx'] ?>');">--><?= $row['user_id_r'] ?><!--</a>--></td>
												<td><!--<a href="javascript:go_view('<?= $row['idx'] ?>');">--><?= $row['user_name_r'] ?><!--</a>--></td>
												...
												<td>
													<? if ($row['orderstat'] == "pre") { ?>
														<a href="javascript:go_payment_complete('<?= $row['order_num'] ?>');" class="btn_green">승인</a>
														<br />
														<a href="javascript:go_payment_return('<?= $row['order_num'] ?>');" class="btn_red">반려</a>
													<? } else { ?>
