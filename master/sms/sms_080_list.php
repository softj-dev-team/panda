...
if ($v_sect) {
	$where .= " and a.cell_num = '" . str_replace("-", "", $v_sect) . "' ";
}
if ($s_sect2) {
...
$query = "SELECT a.*, spam_080_group.group_num FROM spam_080 a INNER JOIN spam_080_group ON a.group_idx = spam_080_group.idx where 1 " . $where . $order_by . " limit " . $StarRowNum . " , " . $EndRowNum;

//echo "<br><br>쿼리 = " . $query . "<br><Br>";

$result = mysqli_query($gconnet, $query);

$query_cnt = "SELECT * FROM spam_080 a INNER JOIN spam_080_group ON a.group_idx = spam_080_group.idx where 1 " . $where;
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function go_regist() {
		location.href = "sms_080_write.php?<?= $total_param ?>";
	}

	function go_regist_080() {
		location.href = "sms_080_group_list.php";
	}
	...
	function go_tot_del() {
		...
		if (check) {
			if (confirm('선택하신 번호를 삭제 하시겠습니까?')) {
				frm.action = "sms_080_delete.php";
				frm.submit();
			}
		} else {
			false;
		...
		<div class="sub_wrap">
			...
			<div class="container clearfix">
				<div class="content">
					...
					<div class="list_tit">
						<h3>080 차단번호 리스트</h3>
						<button class="btn_add" onclick="go_regist_080();" style="width:150px; right:180px;"><span>080 목록/등록</span></button>
						<button class="btn_add" onclick="go_regist();" style="width:150px;"><span>차단번호 등록</span></button>
					</div>
					<div class="list">
						<!-- 검색창 시작 -->
						<table class="search">
