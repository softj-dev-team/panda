...
from sms_save a INNER JOIN member_info ON a.member_idx = member_info.idx where 1 " . $where . $order_by . " limit " . $StarRowNum . " , " . $EndRowNum;
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function go_down() {
		location.href = "sms_send_down.php?<?= $total_param ?>";
	}
			...
			<div class="container clearfix">
				<div class="content">
					...
					<div class="list">
						<!-- 검색창 시작 -->
						<table class="search">
							<form name="s_mem" id="s_mem" method="get" action="sms_send_list.php">
										...
										?>
											<tr>
												...
												<td><?php
													if ($row['reserv_yn'] == "Y") {
														echo "예약";
														echo "<br/>" . $row['reserv_date'] . "<br/>" . $row['reserv_time'] . ":" . $row['reserv_minute'];
													} else {
														echo "바로";
													}
													?></td>
												...
												<td><?= $row['user_name'] ?><br />(<?= $row['user_id'] ?>)</td>
												<td><?= $row['cell_send'] ?></td>
												<td><?= $section ?></td>
                    ...
                    <td>비고</td>-->
												<td>
													<a href="./total_sms_send_list.php?save_idx=<?= $row['idx'] ?>" style="padding:3px 5px; border:1px solid gray;">발송내역보기</a>
												</td>
											</tr>
										<? } ?>
