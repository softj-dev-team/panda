...
$query = "SELECT * FROM spam_list a where 1 " . $where . $order_by . " limit " . $StarRowNum . " , " . $EndRowNum;

//echo "<br><br>쿼리 = " . $query . "<br><Br>";

$result = mysqli_query($gconnet, $query);

$query_cnt = "SELECT * FROM spam_list a where 1 " . $where;
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function go_regist() {
		location.href = "sms_spam_write.php?<?= $total_param ?>";
	}
	...
	function go_tot_del() {
		...
		if (check) {
			if (confirm('선택하신 번호를 삭제 하시겠습니까?')) {
				frm.action = "sms_spam_delete.php";
				frm.submit();
			}
		} else {
			false;
