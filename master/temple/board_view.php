...
$sql = "SELECT *,(select ad_url from ad_info where 1 and idx=a.product_idx) as ad_url,(select ad_title from ad_info where 1 and idx=a.product_idx) as ad_title,(select tag_value from ad_info_add where 1 and cate_type='cate' and ad_info_idx=a.product_idx order by idx desc limit 0,1) as cate_code1,(select member_idx from ad_info where 1 and idx=a.product_idx) as ad_member_idx FROM board_content a where 1=1 and a.idx = '".$idx."' and a.bbs_code='".$bbs_code."'";
...
################## 조회수 관리 시작 ############################
set_vcnt_up("board_content",$row['bbs_code'],$row['idx'],$row['member_idx'],$_SESSION['admin_coinc_idx'],"board_content","cnt"); // 조회수 증가
...
switch ($row[bbs_sect]) {
	...
	case "news" : 
				...
				$sql_sub_notice_1 = "select user_name from member_bisut_info where 1 and member_idx='".$row[ad_member_idx]."'";
				...
				$ad_cate_name2 = $row_sub_notice_1['user_name'];
...
<div id="wrap" class="skin_type01">
	...
	<div class="sub_wrap">
		...
		<?if($bbs_code == "gen" || $bbs_code == "teach" || $bbs_code == "adreview"){
