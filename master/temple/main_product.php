...
################## 파라미터 조합 #####################
$total_param = 'bmenu='.$bmenu.'&smenu='.$smenu.'&field='.$field.'&keyword='.$keyword.'&v_sect='.$v_sect.'&s_gubun='.$s_gubun.'&s_level='.$s_level.'&s_gender='.$s_gender.'&s_sect1='.$s_sect1.'&s_sect2='.$s_sect2.'&s_order='.$s_order.'&reltype='.$reltype;
		...
		<div class="content" style="position:relative; padding:0 10px 0 10px;">
			...
			<form action="main_pro_action.php" method="post" name="frm2" id="frm2" >
				...
				<input type="hidden" name="product_idx" value=""/>
					<? if($num==0) { ?>
							<tr>
								<td colspan="10" height="40">등록된 사찰이 없습니다.</strong></td>
							</tr>
						<? } ?>
						<?
						...
						?>
					<tr>
						<td><input type="radio" name="temple_idx" value="<?=$row[idx]?>" required="yes"  message="사찰" <?if($row[idx] == $proidx){?>checked<?}?>></td>
						<td><a href="javascript:go_view('<?=$row[idx]?>');">
							...
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$row['temple_title']?></a></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$row['temple_url']?></a></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=get_temple_layout($row['temple_layout'])?></a></td>
							<td><a href="javascript:go_view('<?=$row[idx]?>');"><?=$row['addr1']?></a></td>
			...
			<div style="text-align:right;padding-right:10px;padding-top:10px;">
				<a href="javascript:go_submit();" class="btn_blue">선택한 사찰로 설정</a>
			</div>
