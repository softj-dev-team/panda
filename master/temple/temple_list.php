...
<!--	 
	function go_view(no){
		location.href = "temple_view.php?idx="+no+"&pageNo=<?=$pageNo?>&<?=$total_param?>";
	}
	...
	function go_regist(){
		location.href = "temple_write.php?<?=$total_param?>";
	}

	function go_regist_add(no){
		location.href = "temple_write_add.php?temple_idx="+no+"&<?=$total_param?>";
	}
	...
	if(check) {
		if(confirm('선택하신 프로그램를 삭제 하시겠습니까?')){
			if(confirm('삭제하신 프로그램는 복구할 수 없습니다. 정말 삭제 하시겠습니까?')){
				frm.action = "temple_list_action_del.php";
				frm.submit();
			}
		}
	} else {
...
var check = chkFrm('frm');
	if(check) {
		...
		frm.action = "temple_list_action_change.php";
		frm.submit();
	} else {
		false;
	}
...
//var check = chkFrm('frm');
	//if(check) {
		frm.action = "temple_list_action_excel.php";
		frm.submit();
	/*} else {
		false;
	}*/
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list_tit">
					<h3>사찰 리스트</h3>
					<button class="btn_add" onclick="go_regist();"><span>사찰 등록</span></button>
				</div>
				<!-- 네비게이션 종료 -->
				<div class="list">
				<!-- 검색창 시작 -->
