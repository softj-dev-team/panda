...
$where = " and member_idx='".$member_idx."' and point_sect='smspay' and mile_sect = 'M' and board_tbname='sms_save'";
	
if($keyword){
	$where .= " and (mile_title like '%".$keyword."%')";
}

if($s_date){
	$where .= " and substring(wdate,1,10) >= '".$s_date."'";
}
...
$query = "select a.* from member_point a where 1 ".$where.$order_by." limit ".$StarRowNum." , ".$EndRowNum;
//echo $query;
$result = mysqli_query($gconnet,$query);

$query_cnt = "select a.idx from member_point a where 1 ".$where;
...
?>
    ...
    <section class="sub">
        ...
        <div class="tlb center border">
			...
			<? if($num==0) { ?>
				<tr>
					<td colspan="4" height="40"><strong>발송내역이 없습니다.</strong></td>
				</tr>
			<? } ?>
			...
			?>
                <tr>
                    <td><?=$row['wdate']?></td>
                    <td><?=number_format($row['chg_mile'],0)?></td>
	                <td><?=$row['mile_title']?></td>
                    <td><?=number_format($row['cur_mile'],0)?></td>
                </tr>
            <?}?>    
