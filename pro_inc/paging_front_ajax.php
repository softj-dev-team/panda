	...
	//echo "시작페이지 = ".$start_page." | 현재페이지 = ".$c_page." || current_page = ".$current_page." ||| 종료페이지 = ".$end_page." |||| 총 페이지수 = ".$total_page."<br><br>";
			...
			###처음 버튼
				if ($c_page > $page_size) {
					//마지막
				?>
					<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=1');" class="start"><img src="/images/pagenation/ll.png"></a>
				<?
					//이전
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$prev_list?>');" class="pre"><img src="/images/pagenation/l.png"></a>
					<?
				}
			...
			###페이지 출력
				for ($i=$start_page;$i <= $end_page;$i++){
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$page?>');" class="atv"><?=$i?></a>
					<?
					}
...
;					}
			...
			###다음버튼
				if($total_page > $end_page){
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$next_list?>');" class="next"><img src="/images/pagenation/r.png"></a>
					<?
					// 처음
					...
					?>
						<a href="javascript:get_data('<?=$target_link?>', '<?=$target_id?>', '<?=$target_param?>&pageNo=<?=$last_page?>');" class="end"><img src="/images/pagenation/rr.png"></a>
					<?
				}
