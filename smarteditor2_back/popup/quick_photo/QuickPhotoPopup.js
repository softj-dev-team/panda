    ...
    function drop(ev) {
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				...
				files2=files[i].name;
				filies3=files2.split('.');
				files_type='image/'+filies3[filies3.length-1];

				if (!rFilter.test(files[i].type) && !rFilter.test(files_type)) {
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/smarteditor2/popup/quick_photo/FileUploader_html5.php'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/smarteditor2/popup/quick_photo/FileUploader.php',	//샘플 URL입니다.
